...
class AwxController(BaseController):
    ...
    def project_get(self):
        ...
        if oid is not None:
            ...
            self.app.render(res, key='projects', headers=self._meta.headers, fields=self._meta.fields)

    @ex(
        help='get job templates',
        description='get job templates',
        arguments=PARGS([
            (['-id'], {'help': 'job template id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def job_template_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='job_templates', headers=self._meta.headers, fields=self._meta.fields)
