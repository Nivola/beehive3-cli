...
class WinAPIPlatformController(BaseController):
    ...
    def pre_command_run(self):
        ...
        if label is None:
            ...
            if len(keys) > 0:
                ...
                raise Exception('No winapi (veeam) default platform is available for this environment. Select '
                                'another environment')
        ...
        winapiconn = {'host': self.conf.get('host') , 'port': self.conf.get('port'), "proto": self.conf.get('proto'),
                          'user': self.conf.get('user'), 'pwd': self.conf.get('pwd'), 'verified': False}
    ...
    def get_jobs_status(self):
        ...
        self.app.render(res_json['data'], headers=['ID', 'JobName', 'Status', 'Type', 'State',
    ...
    def get_jobs_id(self):
        ...
        str_json = json.loads(res.decode('utf-8'))
        self.app.render(str_json['data']['Job'], headers=['Name', 'Description', 'JobType', 'ScheduleConfigured',
                                                          'ScheduleEnabled', 'NextRun'])
