...
class VsphereController(BaseController):
    class Meta:
        ...
        ds_headers = ['id', 'uuid', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'accessible',
                      'maintenanceMode', 'freespace', 'type']
        ds_fields = ['id', 'uuid', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.accessible',
                     'details.maintenanceMode', 'details.freespace', 'details.type']

        server_headers = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'runstate',
                          'ip-address', 'hostname', 'cp', 'ram', 'disk', 'is-template']
        server_fields = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'details.state',
                         'details.ip_address', 'details.hostname', 'details.cpu', 'details.ram', 'details.disk',
                         'details.template']

        flavor_headers = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'vcpus', 'ram', 'disk',
                          'version', 'guest_id']
        flavor_fields = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'details.vcpus', 'details.ram',
                         'details.disk', 'details.version', 'details.guest_id']

        vt_headers = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'disk_iops', 'tag']
        vt_fields = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'details.disk_iops', 'tag']

        vol_headers = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'status', 'size', 'bootable',
                       'encrypted', 'volume_type']
        vol_fields = ['id', 'uuid', 'ext_id', 'name', 'parent', 'container', 'state', 'details.status', 'details.size',
                      'details.bootable', 'details.encrypted', 'details.volume_type']
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'datacenter id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def datacenter_get(self):
        ...
        if oid is not None:
            ...
            self.app.render(res, key='datacenters', headers=self._meta.headers, fields=self._meta.fields)
    ...
    def datastore_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='datastores', headers=self._meta.ds_headers, fields=self._meta.ds_fields)
    ...
    def cluster_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='clusters', headers=self._meta.headers, fields=self._meta.fields)

    @ex(
        help='get distributed virtual switches',
        description='get distributed virtual switches',
        arguments=PARGS([
            (['-id'], {'help': 'distributed virtual switch id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def dvs_get(self):
        oid = getattr(self.app.pargs, 'id', None)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'distributed virtual port group id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def dvpg_get(self):
        oid = getattr(self.app.pargs, 'id', None)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'nsx manager id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def nsx_get(self):
        oid = getattr(self.app.pargs, 'id', None)
    ...
    def nsx_security_group_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='nsx_security_groups', headers=self._meta.headers, fields=self._meta.fields)
    ...
    def __edge_nat_config(self, features):
        ...
        self.c('\nNAT', 'bold')
        self.c('\nrules', 'underline')
        headers = ['id', 'desc', 'type', 'action', 'vnic', 'enabled', 'origAddr', 'origPort',
                   'transAddress', 'transPort', 'dnatMatchSourceAddr', 'dnatMatchSourcePort',
                   'isLogged', 'enabled', 'protocol']
    ...
    def __edge_lb_config(self, load_balancer):
        ...
        headers = ['applicationProfileId', 'name', 'insertXForwardedFor', 'sslPassthrough', 'template',
                   'serverSslEnabled', 'persistence.method']
        ...
        if not pools:
            ...
            def print_members(members):
                ...
                res = []
                for m in members:
                    res.append('id:{memberId}, name:{name}, ip:{ipAddress}, weight:{weight}, '
            ...
            self.app.render(pools, table_style='grid', headers=headers, transform=transform, maxsize=1000)

        self.c('\nvirtual server', 'underline')
        headers = ['virtualServerId', 'name', 'enabled', 'ipAddress', 'protocol', 'port', 'connectionLimit',
                   'connectionRateLimit', 'defaultPoolId', 'applicationProfileId', 'enableServiceInsertion',
                   'accelerationEnabled']
        self.app.render(virtual_server, table_style='grid', headers=headers, maxsize=50)

    def __edge_routing_config(self, routing, ospf):
    ...
    def nsx_edge_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                feature_list = [{'feature': k, 'version': v['version'], 'enabled': v['enabled']}
                                for k, v in features.items() if isinstance(v, dict) is True]

                self.app.render(res, details=True)
                self.c('\nbase configuration', 'underline')
                self.app.render(details, details=True)
                self.c('\nsyslog', 'underline')
                self.app.render(syslog, headers=['ipAddress', 'protocol'],
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='nsx_edges', headers=self._meta.headers, fields=self._meta.fields)
    ...
    def server_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.c('\nnetworks', 'underline')
                self.app.render(networks, headers=['name', 'mac_addr', 'dns', 'fixed_ips', 'net_id', 'port_state'])
                self.c('\nsecurity groups', 'underline')
                self.app.render(sgs, headers=['id', 'uuid', 'name'])
                self.c('\nvolumes', 'underline')
                headers = ['uuid', 'name', 'name', 'disk_object_id', 'storage', 'size', 'unit_number', 'bootable',
                           'thin', 'mode']
                headers = ['uuid', 'name', 'vsphere:name', 'disk_object_id', 'storage', 'size', 'unit_number',
                           'bootable', 'thin', 'mode']
    ...
    def server_hw(self):
        ...
        if self.is_output_text():
            ...
            self.app.render(res, details=True)
            self.c('\nnetwork', 'underline')
            self.app.render(network, headers=['type', 'name', 'key', 'connected', 'network.name', 'network.dvs',
                                              'network.vlan', 'macaddress'])
            self.c('\nstorage', 'underline')
            self.app.render(storage, headers=['type', 'name', 'size', 'datastore.file_name', 'datastore.disk_mode',
                                              'datastore.write_through'], maxsize=200)
            self.c('\nfile layout', 'underline')
            self.app.render(file_layout, details=True)
            self.app.render(files, headers=['accessible', 'name', 'uniqueSize', 'key', 'type', 'size'], maxsize=200)
    ...
    def server_guest(self):
        ...
        if self.is_output_text():
            ...
            self.c('\nnics', 'underline')
            self.app.render(nics, headers=['netbios_config', 'network', 'dnsConfig', 'connected', 'ip_config',
                                           'mac_address', 'device_config_id'])
            self.c('\nip_stacks', 'underline')
            for item in ip_stack:
                self.app.render(item.get('dns_config'), headers=['dhcp', 'search_domain', 'hostname', 'ip_address',
                                                                 'domainname'])
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-volume_num'], {'help': 'volume number', 'action': 'store', 'type': int, 'default': 2}),
        ])
    )
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
            (['volume'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['-propagate'], {'help': 'if true detach and delete volume', 'action': 'store', 'type': str,
                              'default': 'true'}),
        ])
    )
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['datacenter'], {'help': 'datacenter id', 'action': 'store', 'type': str, 'default': None}),
            (['-core_x_socket'], {'help': 'core x socket', 'action': 'store', 'type': int, 'default': 1}),
            (['vcpus'], {'help': 'vcpus', 'action': 'store', 'type': str, 'default': None}),
            (['-guest_id'], {'help': 'guest id', 'action': 'store', 'type': str, 'default': 'centos64Guest'}),
            (['ram'], {'help': 'ram', 'action': 'store', 'type': int, 'default': None}),
            (['disk'], {'help': 'disk', 'action': 'store', 'type': int, 'default': None}),
            (['-version'], {'help': 'version', 'action': 'store', 'type': str, 'default': 'vmx-11'}),
        ])
    )
    def flavor_add(self):
    ...
    def volumetype_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.c('\ndatastore', 'underline')
                self.app.render(res, headers=['id', 'uuid', 'name', 'state', 'tag', 'size', 'freespace'])
            else:
                self.app.render(res, details=True)
        else:
            params = []
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='volumetypes', headers=self._meta.vt_headers, fields=self._meta.vt_fields)
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['name'], {'help': 'flavor name', 'action': 'store', 'type': str, 'default': None}),
            (['datacenter'], {'help': 'datacenter id', 'action': 'store', 'type': str, 'default': None}),
            (['-disk_iops'], {'help': 'disk_iops. -1 set no limit', 'action': 'store', 'type': int, 'default': -1}),
        ])
    )
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['-tag'], {'help': 'datastore tag', 'action': 'store', 'type': str, 'default': 'default'}),
        ])
    )
    ...
    def volume_get(self):
        ...
        if oid is not None:
            ...
            uri = '%s/volumes' % self.baseuri
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='volumes', headers=self._meta.vol_headers, fields=self._meta.vol_fields)

    @ex(
        help='add volume',
        description='add volume',
        arguments=PARGS([
            (['container'], {'help': 'vsphere container id', 'action': 'store', 'type': str}),
            ...
            (['volume_type'], {'help': 'volume type', 'action': 'store', 'type': str}),
        ])
    )
    def volume_add(self):
        container = self.app.pargs.container
