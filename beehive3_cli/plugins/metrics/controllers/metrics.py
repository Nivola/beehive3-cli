...
class MetricsController(MetricsBaseController):
    class Meta:
        ...
        backup_headers = [
            ...
            "uuid",
            "name",
            "compute_service_uuid",
            "compute_service_name",
            "trilio_used_capacity [GB]",
            "veeam_used_capacity [GB]",
        ]
        backup_headers_fields = [
            "id",
            "uuid",
            "name",
            "compute_service.uuid",
            "compute_service.name",
            "compute_service.trilio_used_capacity.tot",
            "compute_service.veeam_used_capacity.tot",
        ]
    ...
    def get_bck(self):
        ...
        for cs in compute_services:
            ...
            if accounts.get(cs["account_id"]) is not None:
                accounts[cs["account_id"]]["compute_service"] = {
                    "uuid": cs["uuid"],
                    "name": cz["name"],
                    "trilio_used_capacity": used_capacity,
                    "veeam_used_capacity": veeam_usage.get(cz_id, {}),
                }
        res = [value for value in accounts.values()]
        self.app.render(
            res,
            headers=self._meta.backup_headers,
            fields=self._meta.backup_headers_fields,
        )
    ...
    def get_monit_metrics(self):
        ...
        self.app.render(
            usage_list,
            headers=self._meta.monit_headers,
            fields=self._meta.monit_headers_fields,
        )

    @ex(help="update-volumes", description="update-volumes", arguments=PARGS([]))
    ...
    @ex(help="update-oracle", description="update-oracle", arguments=PARGS([]))
    ...
    def get_accounts(self, oid=None):
        ...
        if oid is not None:
            ...
            account = self.cmp_get(uri, data=data).get("account", [])
            accounts = [account]
        else:
    ...
    def get_veeam_usage(self):
        tenants_usage = {}
        uri = self.config["orchestrators"]["veeam"].get("usage_uri", None)
    ...
    def get_trilio_usage(self):
        ...
        (
            ...
            project,
            domain,
        ) = self.get_openstack_config_settings()
        ...
        if uri and user and password is not None:
            ...
            raise Exception("OpenstackError: missing config settings.")
    ...
    def get_zabbix_config_settings(self):
        ...
        uri = self.config["orchestrators"]["zabbix"].get("uri", None)
        user = self.config["orchestrators"]["zabbix"].get("user", None)
        password = self.config["orchestrators"]["zabbix"].get("pwd", None)
    ...
    def get_openstack_config_settings(self):
        ...
        uri = self.config["orchestrators"]["openstack"].get("uri", None)
        region = self.config["orchestrators"]["openstack"].get("region", None)
        user = self.config["orchestrators"]["openstack"].get("user", None)
        password = self.config["orchestrators"]["openstack"].get("pwd", None)
        project = self.config["orchestrators"]["openstack"].get("project", None)
        domain = self.config["orchestrators"]["openstack"].get("domain", None)
    ...
    def get_metric(self, service_instance_id, metric_type, creation_date):
        ...
        data = {"service_instance_id": service_instance_id, "metric_type": metric_type}
    ...
    def del_metric(self, service_uuid, metricid):
        try:
            ...
            data = {
                "metric": {
                    "metric_oid": str(metricid),
                    "service_instance_oid": str(service_uuid),
                }
            }
    ...
    def add_metric(self, metrictype_id, value, service_uuid, date):
        ...
        data = {
            "metric": {
                "value": value,
                "metric_type_id": "%s" % metrictype_id,
                "metric_num": "0",
                "service_instance_oid": "%s" % service_uuid,
                "job_id": "1",
                "creation_date": date,
            }
        }
    ...
    def get_monit_list(self, *args):
        ...
        def cross_data_cz_cs(resources, services):
            ...
            for r in resources:
                for s in services:
                    if r["uuid"] == s["resource_uuid"]:
                        # ds = DataSupport(
                        #     r[u'desc'],
                        #     s[u'uuid']
                        # )
                        ds = {
                            "name": r.get("desc", None),
                            "account_uuid": s.get("account", {}).get("uuid", None),
                            "account_id": s.get("account_id", None),
                            "compute_service_id": s.get("id", None),
                            "compute_service_uuid": s.get("uuid", None),
                            "match_found": False,
                            "monit_hosts": 0,
                        }
                        res.append(ds)
        ...
        def cross_data_czcs_groups(accounts, zabbix_groups):
            ...
            for account_item in accounts:
                for group_item in zabbix_groups:
                    ...
                    if full_name_hyperv == group_item["name"]:
                        account_item["match_found"] = True
                        account_item["monit_hosts"] += int(group_item["hosts"])
                    if not account_item["match_found"]:
                        for hypervisor in ["vmware", "openstack"]:
                            full_name_hyperv = "%s-%s" % (
                                account_item["name"],
                                hypervisor,
                            )
                            if full_name_hyperv == group_item["name"]:
    ...
    def get_zabbix_usage(self):
        ...
        if uri and user and password is not None:
            ...
            raise Exception("ZabbixError: missing configuration settings.")
        print("Getting hosts monitored by account in '%s' environment ..." % self.env)  # for debug purposes
        # get list of Zabbix hostgroups
        res = zabbix_client.hostgroup.list(selecthosts="hostids")
        # print(jsonDumps(res, ensure_ascii=False, indent=2 ))
        for item in res:
            host_lst = item.get("hosts", {})
            if len(host_lst) > 0:
                item["hosts"] = "%s" % str(len(host_lst))
    ...
    def get_volumes(self):
        ...
        data = {"size": -1}
        data = urlencode(data)
        uri = "%s/provider/volumes" % self.baseuri
        res = self.cmp_get(uri, data=data, timeout=120)
        return res.get("volumes", None)

    def get_db_connection(self):
        conf = self.config["db"].get("service", None)
        host = conf.get("host")
        port = conf.get("port")
        db = conf.get("db")
        pwd = conf.get("pwd")
        db_uri = "mysql+pymysql://%s:%s@%s:%s/%s" % ("service", pwd, host, port, db)
    ...
    def add_or_update_volume_item(self, dbconn, voldesc):
        ...
        resource_uuid = voldesc.get("uuid", None)
        modified_date = voldesc.get("date", {}).get("modified", None)
        ...
        if resource_uuid is not None:
            ...
            try:
                ...
                row = res.fetchone()
                if row is None:
                    creation_date = voldesc.get("date", {}).get("creation", None)
                    instance_resource_uuid = voldesc.get("instance", {}).get("uuid", None)
                    name = voldesc.get("name", None)
                    desc = voldesc.get("desc", None)
                    state = voldesc.get("state", None)
                    size = voldesc.get("size", None)
                    container_name = voldesc.get("container", {}).get("name", None)
                    ...
                    print(
                        "Volume resource_uuid: {resource_uuid} already up to date".format(resource_uuid=resource_uuid)
                    )
    ...
    def get_oracle_dbs(self):
        ...
        data = {"size": -1}
        data = urlencode(data)
        uri = "%s/provider/instances" % self.baseuri
        res = self.cmp_get(uri, data=data, timeout=120)
    ...
    def get_compute_instance(self):
        self.configure_cmp_service()
        data = {"plugintype": "ComputeInstance", "size": -1}
        data = urlencode(data)
        uri = "%s/serviceinsts" % self.baseuri
        res = self.cmp_get(uri, data=data)
        res = res["serviceinsts"]
    ...
    def add_or_update_ora_dbitem(self, dbconn, dbdesc):
        ...
        uuid = dbdesc.get("uuid", None)
        creation_date = dbdesc.get("date", {}).get("creation", None)
        modified_date = dbdesc.get("date", {}).get("modified", None)
        if uuid is not None:
            # trans = None
            res = None
            try:
