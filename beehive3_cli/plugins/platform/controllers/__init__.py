...
class ChildPlatformController(BaseController):
    ...
    def _query(self, index, query, page, size, sort, pretty=True, header=[], field=[], render=True,
               maxsize=1000, transform=None):
        ...
        self.app.log.debug('query result: %s' % truncate(res))
        hits = res.get('hits', {})
        values = []
        headers = fields = []
        ...
        if render is True:
            if pretty:
                ...
                data['values'].reverse()
                self.app.render(data, key='values', headers=header, fields=field, maxsize=maxsize, transform=transform)
