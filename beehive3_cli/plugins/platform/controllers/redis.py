...
def REDIS_ARGS(*list_args):
    orchestrator_args = [
        ...
        (['-D', '--database'], {'action': 'store', 'dest': 'database', 'help': 'redis database number', 'default': 0})
    ]
...
class RedisController(BaseK8sController):
    ...
    def pre_command_run(self):
        ...
        if label is None:
            ...
            if len(keys) > 0:
                ...
                raise Exception('No redis default platform is available for this environment. Select '
                                'another environment')
    ...
    def run_cmd(self, func, dbs=[0], print_res=True, format_res=True):
        ...
        try:
            ...
            if self.sentinel_conf is not None:
                server = RedisManager(None, sentinels=[(s, sentinel_port) for s in sentinels],
                                      sentinel_name=sentinel_name, sentinel_pwd=sentinel_pwd, pwd=pwd, db=self.db)
                res = func(server)
            else:
                if pwd is not None:
    ...
    def summary(self):
        def func(server):
            ...
            for k, v in res.items():
                raw = {}
                for k1 in ['role', 'redis_version', 'process_id', 'uptime_in_seconds', 'os', 'connected_clients',
                           'total_commands_processed', 'pubsub_channels', 'total_system_memory_human',
                           'used_memory_rss_human', 'used_memory_human', 'used_cpu_sys', 'used_cpu_user',
    ...
    @ex(
        ...
        arguments=REDIS_ARGS([
            (['-port'], {'help': 'redis port', 'action': 'store', 'default': 443}),
            (['-pattern'], {'help': 'keys search pattern [default=*]', 'action': 'store', 'default': 443,
                            'default': '*'}),
        ])
    )
    def inspect(self):
    ...
    @ex(
        ...
        arguments=REDIS_ARGS([
            (['-port'], {'help': 'redis port', 'action': 'store', 'default': 443}),
            (['-db'], {'help': 'redis db [default=0]', 'action': 'store', 'default': 0}),
            (['-pattern'], {'help': 'keys search pattern [default=*]', 'action': 'store', 'default': '*'}),
            (['-cursor'], {'help': 'cursor [default=0]', 'action': 'store', 'default': 0}),
            (['-count'], {'help': 'cursor [default=10]', 'action': 'store', 'default': 1000}),
        ])
    )
    def scan(self):
        pattern = self.app.pargs.pattern
    ...
    @ex(
        ...
        arguments=REDIS_ARGS([
            (['seconds'], {'help': 'min ttl accepted', 'action': 'store', 'default': 443, 'type': int}),
            (['-port'], {'help': 'redis port', 'action': 'store', 'default': 443}),
            (['-db'], {'help': 'redis db [default=0]', 'action': 'store', 'default': 0}),
            (['-pattern'], {'help': 'keys search pattern [default=*]', 'action': 'store', 'default': '*'}),
            (['-cursor'], {'help': 'cursor [default=0]', 'action': 'store', 'default': 0}),
            (['-count'], {'help': 'cursor [default=10]', 'action': 'store', 'default': 50}),
        ])
    )
    def deletes(self):
    ...
    @ex(
        ...
        arguments=REDIS_ARGS([
            ...
            (['-pattern'], {'help': 'keys search pattern [default=*]', 'action': 'store', 'default': '*'}),
            (['-cursor'], {'help': 'cursor [default=0]', 'action': 'store', 'default': 0}),
            (['-count'], {'help': 'count [default=10]', 'action': 'store', 'default': 1000}),
            (['-value'], {'help': 'if True show cache value', 'action': 'store', 'default': False, 'type': bool}),
        ])
    )
    def cache_get(self):
        pattern = self.app.pargs.pattern
