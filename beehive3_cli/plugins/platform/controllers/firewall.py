...
class FirewallLogController(ChildPlatformController):
    ...
    @ex(
        ...
        arguments=PLATFORM_ARGS(PAGINATION_ARGS, [
            (['-index'], {'help': 'index name', 'action': 'store', 'type': str, 'default': None}),
            (['-reject'], {'help': 'if true show only reject', 'action': 'store', 'type': str, 'default': 'true'}),
            (['-sort'], {'help': 'sort field. Ex. @timestamp:desc', 'action': 'store', 'type': str,
                         'default': '@timestamp:desc'}),
            (['-pretty'], {'help': 'if true show pretty logs', 'action': 'store', 'type': str, 'default': 'true'}),
            (['-ip'], {'help': 'ip address', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def dfw(self):
    ...
    @ex(
        ...
        arguments=PLATFORM_ARGS(PAGINATION_ARGS, [
            (['edge'], {'help': 'edge name', 'action': 'store', 'type': str, 'default': None}),
            (['-type'], {'help': 'log type: firewall, config, nat, ', 'action': 'store', 'type': str, 'default': None}),
            (['-index'], {'help': 'index name', 'action': 'store', 'type': str, 'default': None}),
            (['-reject'], {'help': 'if true show only reject', 'action': 'store', 'type': str, 'default': 'true'}),
            (['-sort'], {'help': 'sort field. Ex. @timestamp:desc', 'action': 'store', 'type': str,
                         'default': '@timestamp:desc'}),
