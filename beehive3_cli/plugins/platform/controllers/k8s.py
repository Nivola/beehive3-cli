...
from beehive3_cli.plugins.platform.util.k8s_util import delete_from_dict, list_from_dict


def K8S_ARGS(*list_args):
    orchestrator_args = [
        (['-C', '--cluster'], {'action': 'store', 'dest': 'cluster', 'help': 'k8s cluster reference label'}),
        (['-N', '--namespce'], {'action': 'store', 'dest': 'namespace', 'help': 'k8s current namespace'}),
    ]
...
class BaseK8sController(ChildPlatformController):
    def pre_command_run(self):
        ...
        self.k8s_deploy_config = '%s/k8s/cmp' % self.ansible_path
        ...
        if label is None:
            ...
            if len(keys) > 0:
                ...
                raise Exception('No k8s default platform is available for this environment. Select '
                                'another environment')
        ...
        self.configuration.host = '%s://%s:%s' % (self.conf.get('proto'), self.k8s_hosts[0],
                                                  self.conf.get('port'))
    ...
    def __get_stream(self, client, pod, namespace):
        log_type = None
        for line in client.read_namespaced_pod_log(pod, namespace, follow=True, tail_lines=100,
    ...
    def get_pod_log(self, namespace, oid=None, name=None, tail_lines=100, follow=False):
        ...
        if oid is not None:
            ...
            for log_pod in log_pods:
                if follow is True:
                    t = Thread(target=self.__get_stream, args=(v1, log_pod, namespace,))
                    t.start()
                    t.join(5)
                else:
    ...
    def print_k8s_response(self, k8s_objects, kinds=None):
        ...
        if kinds is not None and len(kinds) == 1:
            kind = kinds[0]
            if kind == 'Deployment':
                headers = ['kind', 'name', 'namespace', 'creation_date', 'role', 'replicas',
                           'container.name', 'container.image', 'container.cmd']
            elif kind == 'Service':
                headers = ['kind', 'name', 'namespace', 'creation_date', 'type', 'cluster_ip', 'node_port',
                           'target_port']
...
class K8sController(BaseK8sController, ElkController):
    ...
    @ex(
        help='ping k8s cluster',
        description='ping k8s cluster',
        arguments=K8S_ARGS()
    )
    def ping(self):
        resp = []
    ...
    @ex(
        ...
        arguments=K8S_ARGS([
            (['-id'], {'help': 'node id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def node_get(self):
        ...
        if node is not None:
            ...
            self.app.render(res, headers=['uid', 'name', 'ip', 'cpu', 'memory', 'images', 'os_image', 'kernel',
                                          'container'])

    @ex(
        help='get k8s namespace',
        description='get k8s namespace',
        arguments=K8S_ARGS()
    )
    ...
    @ex(
        help='get k8s service',
        description='get k8s service',
        arguments=K8S_ARGS()
    )
    def service_get(self):
        ...
        if namespace == 'all':
            ...
            fields = ['metadata.name', 'metadata.creation_timestamp', 'spec.type', 'metadata.namespace',
                      'spec.cluster_ip', 'spec.ports']
            self.app.render(res, headers=headers, fields=fields, transform=transform, maxsize=200)
        else:
            ...
            fields = ['metadata.name', 'metadata.creation_timestamp', 'spec.type', 'spec.cluster_ip', 'spec.ports']
            self.app.render(res, headers=headers, fields=fields, transform=transform, maxsize=200)
    ...
    @ex(
        ...
        arguments=K8S_ARGS([
            (['-id'], {'help': 'deployment app id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def deploy_get(self):
        app = self.app.pargs.id
        namespace = self.get_namespace()
        v1 = client.AppsV1Api(self.client)
        if app is not None and namespace is not None:
            ...
            self.app.render(conditions, headers=['type', 'status', 'reason', 'last_update_time',
                                                 'last_transition_time', 'message'], maxsize=200)

        elif namespace == 'all':
            apps = v1.list_deployment_for_all_namespaces()
            res = apps.to_dict().get('items', [])
            headers = ['name', 'namespace', 'creation', 'replicas', 'available replicas', 'ready replicas',
                       'updated replicas']
            fields = ['metadata.name', 'metadata.namespace', 'metadata.creation_timestamp', 'status.ready_replicas',
                      'status.available_replicas', 'status.replicas', 'status.updated_replicas']
            ...
            for i in res:
                ...
                for c in i['status']['conditions']:
                    ...
                    c['last_update_time'] = str(c['last_update_time'])
            transform = {'metadata.creation_timestamp': lambda x: str(x)}
            headers = ['name', 'creation', 'replicas', 'available replicas', 'ready replicas', 'updated replicas']
            fields = ['metadata.name', 'metadata.creation_timestamp', 'status.ready_replicas',
                      'status.available_replicas', 'status.replicas', 'status.updated_replicas']
    ...
    @ex(
        ...
        arguments=K8S_ARGS([
            (['-id'], {'help': 'pod id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def pod_get(self):
        ...
        if pod is not None and namespace is not None:
            ...
            transform = {'volume_mounts': lambda x: '\n'.join([n['name'] for n in x]),
                         'command': lambda x: '\n'.join(x)}
            ...
            self.app.render(status, details=True)
            self.c('\nstatus - conditions', 'underline')
            self.app.render(conditions, headers=['type', 'status', 'reason', 'last_transition_time', 'message'],
                            maxsize=200)
    ...
    def pod_elk_log_count(self):
        ...
        for item in pods:
            ...
            query = {
                'match': {'kubernetes.pod.name': {'query': name, 'operator': 'and'}}
            }
            body = {'query': query}
    ...
    @ex(
        ...
        description='get k8s pod',
        arguments=K8S_ARGS([
            (['-name'], {'help': 'pod name like', 'action': 'store', 'type': str, 'default': None}),
            (['-id'], {'help': 'pod id', 'action': 'store', 'type': str, 'default': None}),
            (['-lines'], {'help': 'the number of lines from the end of the logs to show', 'action': 'store',
                          'type': int, 'default': 20}),
            (['-follow'], {'help': 'follow the log stream of the pod', 'action': 'store_true'}),
        ])
    )
    def pod_log(self):
        pod = self.app.pargs.id
    ...
    @ex(
        ...
        arguments=K8S_ARGS([
            (['id'], {'help': 'pod id', 'action': 'store', 'type': str, 'default': None}),
            (['-cmd'], {'help': 'command to run', 'action': StringAction, 'type': str, 'nargs': '+', 'default': ''})
        ])
    )
    def pod_connect(self):
        ...
        console = stream(v1.connect_get_namespaced_pod_exec,
                         pod,
                         namespace,
    ...
    @ex(
        ...
        arguments=K8S_ARGS([
            (['app'], {'help': 'app name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def app_get(self):
