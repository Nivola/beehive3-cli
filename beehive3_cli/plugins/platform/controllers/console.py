...
def CONSOLE_ARGS(*list_args):
    orchestrator_args = [
        (['-C', '--console'], {'action': 'store', 'dest': 'console', 'help': 'console label'})
    ]
...
class ConsoleController(ChildPlatformController):
    ...
    def versions(self):
        ...
        cmd = '&&'.join(cmds)
        res = client.cmd(cmd, timeout=5.0)
        res = res.get('stdout')
    ...
    @ex(
        ...
        arguments=CONSOLE_ARGS([
            (['-pkgs'], {'help': 'user name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def update(self):
    ...
    def get_user_env_file_version(self, client, user, env):
        res = client.cmd('head -1 %s/%s/%s/config/env/%s' % (self.user_path, user, self.user_config_path, env),
                         timeout=5.0)
    ...
    def user_list(self):
        ...
        client = self.__get_ssh_client()
        res = client.cmd('ls %s' % self.user_path, timeout=5.0)
        users = res.get('stdout')
        res = client.cmd('ps -efa |grep python3|grep -v grep|awk \'{print($1)}\'', timeout=5.0)
        from collections import Counter
        procs = Counter(res.get('stdout'))
        idx = 1
        for user in users:
            res = client.cmd('ls %s/%s/%s/config/env/' % (self.user_path, user, self.user_config_path), timeout=5.0)
            ...
            if res.get('stderr') != '':
                ...
                item = {'idx': idx, 'user': user, 'cmp_user': cmp_user_name, 'envs': ','.join(final_envs)}
    ...
    @ex(
        ...
        description='list user configured',
        arguments=CONSOLE_ARGS([
            (['user'], {'help': 'user name', 'action': 'store', 'type': str, 'default': 'user'}),
        ])
    )
    def user_get(self):
        user = self.app.pargs.user
        ...
        self.c('\nsessions', 'underline')
        sessions = client.cmd('ps -efa |grep python3|grep -v grep|awk \'{print($1,$5,$7,$13)}\'|grep %s' % user,
                              timeout=5.0).get('stdout')
        sessions = [s.split(' ') for s in sessions]
        sessions = [{'date': s[1], 'time': s[2], 'node': s[3]} for s in sessions]

        self.c('\nconsole management', 'underline')
        exists = client.cmd('ls %s/%s/beehive-mgmt' % (self.user_path, user), timeout=5.0)
    ...
    def __add_environment(self, sshclient, env, sshuser):
        ...
        remote_filepath = '%s/%s/%s/config/env' % (self.user_path, sshuser, self.user_config_path)
    ...
    @ex(
        ...
        arguments=CONSOLE_ARGS([
            (['user_ssh'], {'help': 'console user', 'action': 'store', 'type': str})
        ])
    )
    def user_setup(self):
        ...
        dirname = '%s/%s/%s/config/env' % (self.user_path, sshuser, self.user_config_path)
        ...
        if self.is_docker:
            ...
            filename = '%s/%s/%s/config/beehive.yml' % (self.user_path, sshuser, self.user_config_path)
            client.file_put('%s/beehive.docker.yml.%s.j2' % (tmpl_config_path, self.console_type), filename)
            print('put file: %s' % filename)
            ...
            print('change file %s acl to: 666' % filename)
            client.cmd('usermod -a -G docker %s' % user, timeout=5.0).get('stdout')
    ...
    @ex(
        ...
        arguments=CONSOLE_ARGS([
            (['user_ssh'], {'help': 'console user', 'action': 'store', 'type': str}),
            (['-isadmin'], {'help': 'console user', 'action': 'store', 'type': str, 'default': 'false'})
        ])
    )
        ...
        if isadmin is True:
            ...
            with SCPClient(ssh.get_transport(), progress=self.__scp_progress) as scp:
                for path in ['configs/console', 'post-install']:
                    local_package_path = '%s/beehive-mgmt/%s' % (self.local_package_path, path)
                    remote_package_path = '%s/%s/beehive-mgmt/%s' % (self.user_path, sshuser, path)
    ...
    @ex(
        ...
        arguments=CONSOLE_ARGS([
            (['user_ssh'], {'help': 'console user', 'action': 'store', 'type': str}),
            (['user_env'], {'help': 'comma separated list of environment to add', 'action': 'store', 'type': str})
        ])
    )
