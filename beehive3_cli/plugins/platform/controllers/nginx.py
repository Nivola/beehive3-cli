    ...
    def pre_command_run(self):
        ...
        if label is None:
            ...
            if len(keys) > 0:
                ...
                raise Exception('No nginx default platform is available for this environment. Select '
                                'another environment')
    ...
    def status(self):
        def func(server):
            try:
                ...
                res = get('https://%s:%s/nginx_status' % (server, self.port), proxies=proxies, verify=False)
                self.app.log.debug('uri: https://%s:%s' % (server, self.port))
                if res.status_code == 200:
        ...
        headers = ['host', 'active', 'accepts', 'handled', 'requests', 'reading', 'writing', 'waiting']
        fields = ['host', 'response.conns.active', 'response.conns.accepts', 'response.conns.handled',
                  'response.conns.requests', 'response.conns.reading', 'response.conns.writing',
