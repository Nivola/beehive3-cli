...
class CheckEngine(object):
    ...
    def mariadb(self):
        ...
        for host in hosts:
            ...
            res = (status['wsrep_cluster_status'] == 'Primary') and \
    ...
    def vsphere(self):
        ...
        self.__stop_time()
        self.__printline([engine, conf.get('vcenter').get('host'), 443, 'vcenter ping', ping, self.elapsed])
        self.__start_time()
        ping = client.system.ping_nsx()
        self.__stop_time()
        self.__printline([engine, conf.get('nsx').get('host'), 443, 'vcenter nsx', ping, self.elapsed])
        self.__start_time()
        ...
        self.__stop_time()
        self.__printline([engine, conf.get('nsx').get('host'), 443, 'vcenter nsx controllers', c_status, self.elapsed])
        self.__start_time()
        ...
        for c, v in components.items():
            if c in ['VPOSTGRES', 'RABBITMQ', 'NSX', 'SSH']:
                if c.get('status') == 'RUNNING':
                    ...
                    c_status = c_status and False
        self.__stop_time()
        self.__printline([engine, conf.get('nsx').get('host'), 443, 'vcenter nsx components', c_status, self.elapsed])
...
class CliPlatformController(BaseController):
    ...
    def pre_command_run(self):
        ...
        self.DEFAULT_ENGINE = ['nginx', 'mariadb', 'k8s', 'vsphere', 'openstack', 'zabbix', 'awx', 'elk']

    @ex(
        help='check platform status',
        description='check platform status',
        arguments=ARGS([
            (['-engines'], {'help': 'engine name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def check(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-engine'], {'help': 'engine name', 'action': 'store', 'type': str, 'default': 'all'}),
        ])
    )
    def version(self):
