...
class CmpTestController(ChildPlatformController):
    ...
    @ex(
        ...
        arguments=ARGS(PAGINATION_ARGS, [
            (['-package'], {'help': 'python package', 'action': 'store', 'type': str, 'default': 'beehive_tests'}),
            (['-plan'], {'help': 'name of the test plan', 'action': 'store', 'type': str, 'default': None}),
            (['-group'], {'help': 'name of the sub test group', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get(self):
        ...
        if package is not None and plan is not None:
            ...
            if group is not None:
                self.app.render([{'package': package, 'test-plan': plan, 'test-group': group, 'idx': p, 'test': i}
                                for p, i in enumerate(getattr(test_run, group))],
                                headers=['package', 'test-plan', 'test-group', 'idx', 'test'])
            else:
                self.app.render([{'package': package, 'test-plan': plan, 'test-group': i} for i in test_groups],
            ...
            for package in packages:
                ...
                test_file = '%s/%s/%s/regression' % \
                            (self.app.config.get('beehive', 'local_package_path'), prj, package)
                try:
    ...
    @ex(
        ...
        arguments=ARGS(PAGINATION_ARGS, [
            ...
            (['-plan'], {'help': 'name of the test plan', 'action': 'store', 'type': str, 'default': None}),
            (['-group'], {'help': 'name of the sub test group', 'action': 'store', 'type': str, 'default': None}),
            (['-list'], {'help': 'list of test id to run', 'action': 'store', 'type': str, 'default': None}),
            (['-test'], {'help': 'name of test to run', 'action': 'store', 'type': str, 'default': None}),
            (['-mainconf'], {'help': 'optional main test configuration', 'action': 'store', 'type': str,
                             'default': None}),
            (['-conf'], {'help': 'optional extra test configuration', 'action': 'store', 'type': str,
                         'default': None}),
            (['-validate'], {'help': 'if True enable api validation in test', 'action': 'store', 'type': str,
                             'default': None}),
            (['-user'], {'help': 'user used to run test. Ex. test1, admin [default=test1]', 'action': 'store',
                         'type': str, 'default': 'test1'}),
            (['-concurrency'], {'help': 'specify how many tests run in parallel for massive test [default=2]',
...
class CmpSubsystemController(BaseK8sController):
    ...
    def __create_subsystem(self, update=False):
        ...
        self.config['mysql'] = {'host': dict_get(mysql, 'hosts.0'), 'port': dict_get(mysql, 'port')}
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-id'], {'help': 'subsystem. Ex. resource, service', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get(self):
        ...
        if oid is None:
            ...
            for subsystem in self._meta.available_subsytems:
                ...
                deploy.extend([{'subsystem': subsystem, 'component': f.rstrip('.yml')}
                               for f in files if f not in ['_res', 'kustomization.yml']])
            ...
            for d in deploy_datas:
                ...
                if kind == 'ConfigMap':
                    ...
                    for c in dict_get(d, 'spec.template.spec.containers'):
                        ...
                        items.append(item)
            self.app.render(items, headers=['kind', 'name', 'role', 'replicas', 'container.name', 'container.image',
                                            'container.cmd'])
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-id'], {'help': 'subsystem. Ex. resource, service', 'action': 'store', 'type': str, 'default': None}),
            (['-role'], {'help': 'deployment role', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def runtime_get(self):
        ...
        if len(pods) > 0:
            self.c('\npods', 'underline')
            headers = ['name', 'status', 'ip', 'host_ip', 'namespace', 'container', 'date']
            fields = ['metadata.name', 'status.phase', 'status.pod_ip', 'status.container_statuses.0.container_id',
                      'status.host_ip', 'status.container_statuses.0.image', 'status.start_time']
    ...
    def runtime_ping(self):
        ...
        headers = ['service', 'host', 'port', 'remote_uri', 'api_ping', 'sql_ping', 'redis_ping', 'redis_identity_ping',
                   'status', 'elapsed']
            ...
            try:
                ...
                if http.status_code == 200:
                    ...
                    if response.get('sql_ping') is True:
                        status = ok
                    res.extend([response.get('uri'), True, sql_ping, status, round(time() - start, 3), True])
            ...
            for path, info in res.get('paths').items():
                for method, info2 in info.items():
                    resp.append({
                        ...
                        'parameters': ','.join([p.get('name', '') for p in dict_get(info2, 'parameters', default=[])])
                    })
    ...
    def runtime_apidocs(self):
        ...
        for k8s_host in self.k8s_hosts:
            ...
            if ping is True:
                resp.append({
                    'url': 'http://%s:%s/apidocs' % (k8s_host, port)
                })
                break
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'subsystem. Ex. resource, service', 'action': 'store', 'type': str, 'default': None}),
            (['role'], {'help': 'deployment role', 'action': 'store', 'type': str, 'default': None}),
            (['-follow'], {'help': 'follow log', 'action': 'store', 'type': str, 'default': 'true'}),
            (['-lines'], {'help': 'log line number', 'action': 'store', 'type': int, 'default': 100}),
        ])
    )
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['file'], {'help': 'subsystem config file full path', 'action': 'store', 'type': str}),
        ])
    )
    def create(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-path'], {'help': 'remote package path', 'action': 'store', 'type': str, 'default': '/opt/cmp'}),
            (['-pkgs'], {'help': 'list of package to sync', 'action': 'store', 'type': str, 'default': 'all'}),
        ])
    )
    def sync(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['subsystem'], {'help': 'subsystem. Ex. resource, service', 'action': 'store', 'type': str}),
            (['-path'], {'help': 'remote package path', 'action': 'store', 'type': str, 'default': '/opt/cmp'}),
            (['-pkgs'], {'help': 'list of package to sync', 'action': 'store', 'type': str, 'default': ''}),
        ])
    )
    def redeploy(self):
        subsystem = self.app.pargs.subsystem
...
class CmpPostInstallController(BaseController):
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-filter'], {'help': 'filter to apply <entity list key>:<key to filter>:<value>. '
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-sections'], {'help': 'comma separated list of section to execute', 'action': 'store', 'type': str,
                             'default': None}),
        ])
    )
    def run(self):
...
class CmpLog2Controller(ChildPlatformController):
    ...
    def get_current_elastic_index(self):
        return '%s-%s-filebeat-7.12.0-%s-%s' % (self.k8s_cluster, self.k8s_namespace,
                                                datetime.now().strftime('%Y.%m.%d'),
                                                self.cmp_cert_postfix)

    def get_current_elastic_event_index(self):
        return 'cmp-event-%s-pylogbeat-2.0.0-%s-%s' % (self.elk_env, datetime.now().strftime('%Y.%m.%d'),
                                                       self.cmp_cert_postfix)
    ...
    @ex(
        ...
        arguments=ARGS(PAGINATION_ARGS, [
            ...
            (['-name'], {'help': 'container partial name. Ex. uwsgi-auth, worker-auth, uwsgi-ssh', 'action': 'store',
                         'type': str, 'default': None}),
            (['-sort'], {'help': 'sort field. Ex. date:desc', 'action': 'store', 'type': str,
                         'default': 'timestamp:desc'}),
            (['-pod'], {'help': 'pod name', 'action': 'store', 'type': str, 'default': None}),
            (['-op'], {'help': 'oepration id. Can be api_id, task_id, task_id:step_name', 'action': 'store',
                       'type': str, 'default': None}),
        ])
    )
    def engine(self):
        ...
        field = ['timestamp', 'host.name', 'kubernetes.container.name', 'kubernetes.pod.name', 'message']
        self._query(index, query, page, size, sort, header=header, field=field, maxsize=5000,
                    transform=transform, pretty=False)
    ...
    @ex(
        ...
        arguments=ARGS(PAGINATION_ARGS, [
            ...
            (['-type'], {'help': 'event id', 'action': 'store', 'type': str, 'default': 'API'}),
            (['-sort'], {'help': 'sort field. Ex. date:desc', 'action': 'store', 'type': str,
                         'default': '@timestamp:desc'}),
        ])
    )
    def event(self):
        ...
        if eventid is None:
            headers = ['event_id', 'opid', 'op', '@timestamp', 'elapsed', 'response', 'user', 'ip', 'dest-ip',
                       'dest-port']
            fields = ['event_id', 'data.opid', 'data.op', '@timestamp', 'data.elapsed', 'data.response.0',
                      'source.user', 'source.ip', 'dest.ip', 'dest.port']
    ...
    @ex(
        ...
        arguments=ARGS(PAGINATION_ARGS, [
            ...
            (['-sort'], {'help': 'sort field. Ex. @timestamp:desc', 'action': 'store', 'type': str,
                         'default': '@timestamp:desc'}),
        ])
    )
    def api(self):
        ...
        if eventid is None:
            headers = ['id', 'api_id', '@timestamp', 'uri', 'elapsed', 'response-code', 'user', 'source',
                       'dest']
            fields = ['event_id', 'data.api_id', '@timestamp', 'data.op', 'data.elapsed', 'data.response.0',
            ...
            if self.is_output_text():
                ...
                self.c('\nworkflow', 'underline')
                headers = ['opid', '@timestamp', 'type', 'operation', 'elapsed', 'response-code', 'user', 'ip-addr',
                           'dest-ip-addr', 'dest-port']
                fields = ['data.opid', '@timestamp', 'type', 'data.op', 'data.elapsed', 'data.response.0',
                          'source.user', 'source.ip', 'dest.ip', 'dest.port']
