    ...
    def __create_groups(self, configs):
        ...
        for obj in dict_get(configs, 'ssh.groups'):
            ...
            for info in users:
                ...
                self.cmp_put(GROUP_URI + '/users', data, 'Set nodegroup %s role %s to user %s' % (name, role, user))
                
            for info in groups:
                group, role = info
                data = {'group': {'group_id': users, 'role': role}}
                self.cmp_put(GROUP_URI + '/groups', data, 'Set nodegroup %s role %s to group %s' % (name, role, group))
    ...
    def __create_keys(self, configs):
        ...
        for obj in dict_get(configs, 'ssh.keys'):
            ...
            for info in users:
                user, role = info
                data = {'user': {'user_id': users, 'role': role}}
                self.cmp_put(GROUP_URI + '/users', data, 'Set key %s role %s to user %s' % (name, role, user))

            for info in groups:
                group, role = info
                data = {'group': {'group_id': users, 'role': role}}
                self.cmp_put(GROUP_URI + '/groups', data, 'Set key %s role %s to group %s' % (name, role, group))
    ...
    def __create_nodes(self, configs):
        ...
        for obj in dict_get(configs, 'ssh.nodes'):
            ...
            for user in nodeusers:
                ...
                exists = self.cmp_exists(NODEUSER_URI+'/'+user['name'], 'Node user %s already exists' % user['name'])
                if exists is False:
                    self.cmp_post(NODEUSER_URI, {'user': user}, 'Add node %s nodeuser %s' % (name, user['username']))

            for group in nodegroups:
                GROUP_URI = '/v1.0/gas/groups/%s/node' % group
                self.cmp_put(GROUP_URI, {'node': obj['name']}, 'Add node %s group %s' % (name, group))

            for info in users:
                user, role = info
                data = {'user': {'user_id': users, 'role': role}}
                self.cmp_put(GROUP_URI + '/users', data, 'Set node %s role %s to user %s' % (name, role, user))

            for info in groups:
                group, role = info
                data = {'group': {'group_id': users, 'role': role}}
                self.cmp_put(GROUP_URI + '/groups', data, 'Set node %s role %s to group %s' % (name, role, group))
