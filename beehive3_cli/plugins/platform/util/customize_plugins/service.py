...
class ServiceCustomizePlugin(CustomizePlugin):
    ...
    def __create_service_catalogs(self, configs):
        ...
        for obj in dict_get(configs, 'service.catalogs'):
            ...
            self.cmp_put('/v1.0/nws/srvcatalogs/%s/defs' % cat['uuid'], {'definitions': {'oids': defs}}, msg)
            ...
            for info in users:
                ...
                self.cmp_post(OBJ_URI + '/users', data, 'Set catalog %s role %s to user %s' % (name, role, user))

            for info in groups:
                group, role = info
                data = {'group': {'group_id': group, 'role': role}}
                self.cmp_post(OBJ_URI + '/groups', data, 'Set catalog %s role %s to group %s' % (name, role, group))
    ...
    def __create_organizations(self, configs):
        ...
        for obj in dict_get(configs, 'authority.organizations'):
            ...
            for info in users:
                user, role = info
                data = {'user': {'user_id': user, 'role': role}}
                self.cmp_post(OBJ_URI + '/users', data, 'Set organization %s role %s to user %s' % (name, role, user))

            for info in groups:
                group, role = info
                data = {'group': {'group_id': group, 'role': role}}
                self.cmp_post(OBJ_URI + '/groups', data, 'Set organization %s role %s to group %s' % (name, role, group))
    ...
    def __create_divisions(self, configs):
        ...
        for obj in dict_get(configs, 'authority.divisions'):
            ...
            for info in users:
                user, role = info
                data = {'user': {'user_id': user, 'role': role}}
                self.cmp_post(OBJ_URI + '/users', data, 'Set division %s role %s to user %s' % (name, role, user))

            for info in groups:
                group, role = info
                data = {'group': {'group_id': group, 'role': role}}
                self.cmp_post(OBJ_URI + '/groups', data, 'Set division %s role %s to group %s' % (name, role, group))
    ...
    def __create_accounts(self, configs):
        ...
        for obj in dict_get(configs, 'authority.accounts'):
            ...
            for info in users:
                user, role = info
                data = {'user': {'user_id': user, 'role': role}}
                self.cmp_post(OBJ_URI + '/users', data, 'Set account %s role %s to user %s' % (name, role, user))

            for info in groups:
                group, role = info
                data = {'group': {'group_id': group, 'role': role}}
                self.cmp_post(OBJ_URI + '/groups', data, 'Set account %s role %s to group %s' % (name, role, group))
