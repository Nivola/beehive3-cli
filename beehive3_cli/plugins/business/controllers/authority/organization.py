...
class OrganizationController(AuthorityControllerChild):
    class Meta:
        ...
        fields = ['uuid', 'name', 'org_type', 'divisions', 'ext_anag_id', 'status', 'date.creation']

    @ex(
        help='get organizations',
        description='get organizations',
        arguments=PARGS([
            (['-id'], {'help': 'organization uuid', 'action': 'store', 'type': str, 'default': None}),
            ...
            (['-org-type'], {'help': 'organization type', 'action': 'store', 'type': str, 'default': None}),
            (['-ext-anag-id'], {'help': 'organization ext_anag_id', 'action': 'store', 'type': str, 'default': None}),
            (['-attributes'], {'help': 'organization attributes', 'action': 'store', 'type': str, 'default': None}),
            (['-hasvat'], {'help': 'organization hasvat', 'action': 'store', 'type': str, 'default': None}),
            (['-partner'], {'help': 'organization partner', 'action': 'store', 'type': str, 'default': None}),
            (['-referent'], {'help': 'organization referent', 'action': 'store', 'type': str, 'default': None}),
            (['-email'], {'help': 'organization email', 'action': 'store', 'type': str, 'default': None}),
            (['-legalemail'], {'help': 'organization legalemail', 'action': 'store', 'type': str, 'default': None}),
            (['-postaladdress'], {'help': 'organization legalemail', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.c('\ndivisions', 'underline')
                headers = ['uuid', 'name', 'organization', 'accounts', 'contact', 'email', 'postaladdress', 'status',
                           'date']
                fields = ['uuid', 'name', 'organization_name', 'accounts', 'contact', 'email', 'postaladdress',
                          'status',
                          'date.creation']
            ...
            params = ['name', 'objid', 'org_type', 'ext_anag_id', 'attributes', 'hasvat', 'partner', 'referent',
                      'email', 'legalemail', 'postaladdress']
            data = self.format_paginated_query(params)
            uri = '%s/organizations' % self.baseuri
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='organizations', headers=self._meta.headers, fields=self._meta.fields)

    @ex(
        help='add organization',
        description='add organization',
        arguments=ARGS([
            (['name'], {'help': 'organization name', 'action': 'store', 'type': str}),
            (['-desc'], {'help': 'organization description', 'action': 'store', 'type': str, 'default': ''}),
            (['-attrib'], {'help': 'organization attributes', 'action': 'store', 'type': str, 'default': None}),            
            (['orgtype'], {'help': 'organization type', 'action': 'store', 'type': str}),
            (['-hasvat'], {'help': 'organization hasvat', 'action': 'store', 'type': str, 'default': False}),            
            (['-ext-anag-id'], {'help': 'organization ext_anag_id', 'action': 'store', 'type': str, 'default': None}),
            (['-partner'], {'help': 'organization partner', 'action': 'store', 'type': str, 'default': False}),
            (['-referent'], {'help': 'organization referent', 'action': 'store', 'type': str, 'default': None}),
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-desc'], {'help': 'organization description', 'action': 'store', 'type': str, 'default': ''}),
            (['-attrib'], {'help': 'organization attributes', 'action': 'store', 'type': str, 'default': None}),
            (['org-type'], {'help': 'organization type', 'action': 'store', 'type': str}),
            (['-hasvat'], {'help': 'organization hasvat', 'action': 'store', 'type': str, 'default': False}),
            (['-ext-anag-id'], {'help': 'organization ext_anag_id', 'action': 'store', 'type': str, 'default': None}),
            (['-partner'], {'help': 'organization partner', 'action': 'store', 'type': str, 'default': False}),
            (['-referent'], {'help': 'organization referent', 'action': 'store', 'type': str, 'default': None}),
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'organization uuid or name', 'action': 'store', 'type': str}),
        ])
    )
    def refresh(self):
        oid = self.app.pargs.id
...
class OrganizationAuthController(AuthorityControllerChild):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'organization uuid', 'action': 'store', 'type': str}),
            (['role'], {'help': 'organization role', 'action': 'store', 'type': str}),
