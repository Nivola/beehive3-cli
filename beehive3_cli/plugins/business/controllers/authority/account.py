...
class AccountController(AuthorityControllerChild):
    class Meta:
        ...
        headers = ['uuid', 'name', 'acronym', 'division', 'contact', 'email_support', 'managed', 'core services',
                   'base services', 'status', 'date']
        fields = ['uuid', 'name', 'acronym', 'division_name', 'contact', 'email_support', 'managed', 'services.core',
                  'services.base', 'status', 'date.creation']
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['-name'], {'help': 'account name', 'action': 'store', 'type': str, 'default': None}),
            (['-division-id'], {'help': 'division uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-contact'], {'help': 'account contact', 'action': 'store', 'type': str, 'default': None}),
            (['-email'], {'help': 'account email', 'action': 'store', 'type': str, 'default': None}),
            (['-email-support'], {'help': 'account email support', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                headers = ['name', 'status', 'services.required', 'services.error', 'services.created',
                           'definitions.required', 'definitions.created']
                fields = ['name', 'status', 'report.services.required', 'report.services.error',
                          'report.services.created', 'report.definitions.required', 'report.definitions.created']
                ...
                fields = ['uuid', 'name', 'plugintype', 'definition_name', 'status', 'resource_uuid', 'is_container',
                          'parent.name', 'date.creation']
                headers = ['id', 'name', 'type', 'definition', 'status', 'resource', 'is_container', 'parent',
                           'creation']
            ...
            self.app.render(res, key='accounts', headers=self._meta.headers, fields=self._meta.fields)
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['-plugintype'], {'help': 'filter by definition plugin', 'action': 'store', 'type': str, 'default': None}),
            (['-category'], {'help': 'filet by category ', 'action': 'store', 'type': str, 'default': None}),
            (['-container'], {'help': 'get only containers definitions', 'action': 'store', 'type': bool, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=PARGS([
            (['id'], {'help': 'account id', 'action': 'store', 'type': str}),
            (['definitions'], {'help': 'comma separated list of definition ids', 'action': 'store', 'type': str,
                               'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-desc'], {'help': 'account description', 'action': 'store', 'type': str, 'default': ''}),
            (['division'], {'help': 'division uuid', 'action': 'store', 'type': str}),
            # (['-price_list'], {'help': 'account price list id', 'action': 'store', 'type': int, 'default': None}),
            (['-contact'], {'help': 'account contact', 'action': 'store', 'type': str, 'default': None}),
            (['-email'], {'help': 'account email', 'action': 'store', 'type': str, 'default': None}),            
            (['-email-support'], {'help': 'account email support', 'action': 'store', 'type': str, 'default': None}),
            (['-email-support-link'], {'help': 'account email support link', 'action': 'store', 'type': str,
                                       'default': None}),
            (['-note'], {'help': 'account note', 'action': 'store', 'type': str, 'default': None}),
            (['-acronym'], {'help': 'account acronym', 'action': 'store', 'type': str, 'default': ''}),
            (['-managed'], {'help': 'if true set account as managed', 'action': 'store', 'type': str, 'default': True}),
        ])
    )
    def add(self):
        data = {
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'account id', 'action': 'store', 'type': str}),
            (['-delete_services'], {'help': 'if true delete all child services', 'action': 'store', 'type': str,
                                    'default': 'true'}),
        ])
    )
    def delete(self):
    ...
    def service_del(self):
        ...
        for item in res:
            ...
            res = self.cmp_delete(uri, data={}, timeout=600, entity='service %s' % item['name'])
            self.wait_for_service(item['uuid'], delta=2)
...
class AccountCapabilityController(AuthorityControllerChild):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'account id', 'action': 'store', 'type': str, 'default': None}),
            (['-capability'], {'help': 'capability name', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def get(self):
        oid = getattr(self.app.pargs, 'id', None)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['id'], {'help': 'account id', 'action': 'store', 'type': str}),
            (['capabilities'], {'help': 'comma separated list of capability name', 'action': 'store', 'type': str})
        ])
    )
    def add(self):
        oid = self.app.pargs.id
...
class AccountTagController(AuthorityControllerChild):
    ...
    def get(self):
        ...
        res = self.cmp_get(uri, data=data)
        self.app.render(res, key='tags', headers=['uuid', 'name', 'desc', 'active', 'creation'],
