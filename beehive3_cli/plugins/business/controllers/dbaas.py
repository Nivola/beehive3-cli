...
class DBServiceInstanceController(BusinessControllerChild):
    ...
    def engines(self):
        ...
        self.app.render(res, headers=['engine', 'version'], fields=['engine', 'engineVersion'], key='engineTypesSet')
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-accounts'], {'help': 'list of account id comma separated', 'action': 'store', 'type': str,
                             'default': None}),
            (['-ids'], {'help': 'list of db instance id comma separated', 'action': 'store', 'type': str,
                        'default': None}),
    ...
    def list(self):
        ...
        headers = ['id', 'name', 'status', 'account', 'Engine', 'EngineVersion', 'AllocatedStorage',
                   'AvailabilityZone', 'DBInstanceClass', 'Subnet', 'Listen', 'Port', 'Date']
        fields = ['DBInstance.%s' % self.__get_field_id_key(), 'DBInstance.%s' % self.__get_field_id_name(),
                  'DBInstance.DBInstanceStatus', 'DBInstance.nvl-ownerAlias', 'DBInstance.Engine',
                  'DBInstance.EngineVersion', 'DBInstance.AllocatedStorage', 'DBInstance.AvailabilityZone',
                  'DBInstance.DBInstanceClass', 'DBInstance.DBSubnetGroup.DBSubnetGroupName',
                  'DBInstance.Endpoint.Address', 'DBInstance.Endpoint.Port', 'DBInstance.InstanceCreateTime']
    ...
    def get(self):
        ...
        if len(res.get('DBInstances')) > 0:
            ...
            if self.is_output_text():
                ...
                network['subnet'] = '%s - %s' % (dict_get(subnet, 'Subnets.0.SubnetIdentifier'),
                ...
                self.app.render(sgs, headers=['groupId', 'groupName'],
                                fields=['VpcSecurityGroupMembership.VpcSecurityGroupId',
    ...
    def __cmp_post(self, data):
        ...
        uuid = dict_get(res, 'CreateDBInstanceResponse.CreateDBInstanceResult.DBInstance.DBInstanceIdentifier')
    ...
    @ex(
        ...
        arguments=ARGS([
            (['name'], {'help': 'db instance name', 'action': 'store', 'type': str}),
            (['account'], {'help': 'parent account id', 'action': 'store', 'type': str}),
            (['type'], {'help': 'db instance type', 'action': 'store', 'type': str}),
            (['subnet'], {'help': 'db instance subnet id', 'action': 'store', 'type': str}),
            (['sg'], {'help': 'db instance security group id', 'action': 'store', 'type': str}),
            (['version'], {'help': 'database engine version', 'action': 'store', 'type': str}),
            (['-pwd'], {'help': 'db root password', 'action': 'store', 'type': str, 'default': None}),
            (['-storage'], {'help': 'data storage capacity in GB', 'action': 'store', 'type': int, 'default': 30}),
            (['-keyname'], {'help': 'ssh key name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def add_mysql(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['sg'], {'help': 'db instance security group id', 'action': 'store', 'type': str}),
            (['version'], {'help': 'database engine version', 'action': 'store', 'type': str}),
            (['-storage'], {'help': 'amount of storage [GB] to allocate for the DB instance', 'action': 'store',
                            'type': int, 'default': 30}),
            (['-pwd'], {'help': 'db root password', 'action': 'store', 'type': str, 'default': None}),
            (['-keyname'], {'help': 'ssh key name', 'action': 'store', 'type': str, 'default': None}),
            (['-postgis'], {'help': 'spatial database extension', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-pwd'], {'help': 'db root password', 'action': 'store', 'type': str, 'default': None}),
            (['-keyname'], {'help': 'ssh key name', 'action': 'store', 'type': str, 'default': None}),
            (['-dbname'], {'help': 'db name', 'action': 'store', 'type': str, 'default': None}),
            (['-lsnport'], {'help': 'listener port', 'action': 'store', 'type': str, 'default': None}),
            (['-archmode'], {'help': 'archivelog mode Y/N', 'action': 'store', 'type': str, 'default': None}),
            (['-partopt'], {'help': 'partitioning option Y/N', 'action': 'store', 'type': str, 'default': None}),
            (['-charset'], {'help': 'character set', 'action': 'store', 'type': str, 'default': 'WE8ISO8859P1'}),
            (['-natcharset'], {'help': 'national charset', 'action': 'store', 'type': str, 'default': None}),
            (['-dbfdisksize'], {'help': 'datafiles disk size in GB', 'action': 'store', 'type': str, 'default': None}),
            (['-recodisksize'], {'help': 'recovery disk size in GB', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def add_oracle(self):
        data = self.__add_common()
        ...
        d = {}
        self.add_field_from_pargs_to_data('pwd', d, 'MasterUserPassword')
        self.add_field_from_pargs_to_data('keyname', d, 'Nvl_KeyName')
    ...
    @ex(
        ...
        arguments=ARGS([
            (['instance'], {'help': 'db instance id', 'action': 'store', 'type': str}),
            (['-dbi_class'], {'help': 'db instance class to set up', 'action': 'store', 'type': str, 'default': None}),
            (['-sg_add'], {'help': 'db instance security group id to add', 'action': 'store', 'type': str,
                           'default': None}),
            (['-sg_del'], {'help': 'db instance security group id to remove', 'action': 'store', 'type': str,
                           'default': None}),
            (['-resize'], {'help': 'new amount of storage (in GiB) to allocate for the db instance', 'action': 'store',
                           'type': int, 'default': None}),
            (['-extensions_add'], {'help': 'db extensions to install, e.g. name1:type1,name2:type2,... where type can '
                                           'be plugin or component', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def update(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['database'], {'help': 'db instance id', 'action': 'store', 'type': str}),
        ])
    )
    def delete(self):
        uuid = self.app.pargs.database
        uri = '%s/databaseservices/instance/deletedbinstance' % self.baseuri
        self.cmp_delete(uri, data={'DBInstanceIdentifier': uuid}, entity='db instance %s' % uuid)
    ...
    def database_get(self):
        ...
        res = dict_get(res, 'DescribeDBInstanceSchemaResponse.DescribeDBInstanceSchemaResult.Schemas')
        self.app.render(res, headers=['db_name', 'size', 'charset', 'collation', 'schemas', 'access_privileges'],
                        maxsize=200, transform={'schemas': lambda x: ','.join(x)})
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['charset'], {'help': 'database charset', 'action': 'store', 'type': str}),
        ])
    )
    ...
    @ex(
        ...
        arguments=ARGS([
            (['instance'], {'help': 'db instance id', 'action': 'store', 'type': str}),
            (['name'], {'help': 'user name', 'action': 'store', 'type': str}),
            (['-force'], {'help': 'force deletion', 'action': 'store', 'type': str, 'default': 'false'}),
        ])
    )
    def user_del(self):
        uuid = self.app.pargs.instance
    ...
    @ex(
        ...
        arguments=ARGS([
            (['instance'], {'help': 'db instance id', 'action': 'store', 'type': str}),
            (['name'], {'help': 'user name', 'action': 'store', 'type': str}),
            (['db_name'], {'help': 'database name. For postgres use db1 to select a database e db1.schema1 to select '
                                   'schema schema1 in database db1', 'action': 'store', 'type': str}),
            (['privileges'], {'help': 'privileges admitted: mysql - SELECT,INSERT,DELETE,UPDATE,ALL, '
    ...
    def user_priv_grant(self):
        ...
        self.cmp_post(uri, data=data, timeout=600)
        self.wait_for_service(uuid)
        self.app.render({'msg': 'grant db instance %s user %s privileges %s to database %s' %
                                (uuid, name, privileges, db_name)})
    ...
    def user_priv_revoke(self):
        ...
        self.cmp_delete(uri, data=data, timeout=600)
        self.wait_for_service(uuid)
        self.app.render({'msg': 'revoke db instance %s user %s privileges %s from database %s' %
                                (uuid, name, privileges, db_name)})
    ...
    @ex(
        ...
        arguments=ARGS([
            (['instance'], {'help': 'virtual machine id', 'action': 'store', 'type': str}),
