...
class AppEngineInstanceController(BusinessControllerChild):
    ...
    @ex(
        ...
        description='get appengine types',
        arguments=ARGS()
    )
    def types(self):
        data = {
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-accounts'], {'help': 'list of account id comma separated', 'action': 'store', 'type': str,
                             'default': None}),
            (['-ida'], {'help': 'list of appengin id comma separated', 'action': 'store', 'type': str,
                        'default': None}),
    ...
    def list(self):
        ...
        headers = ['id', 'name', 'status', 'account', 'Engine', 'EngineVersion',
                   'AvailabilityZone', 'Subnet', 'Uris',  'Date']
        fields = ['instanceId', 'name', 'instanceState.name', 'OwnerAlias', 'engine', 'version',
                  'placement.availabilityZone', 'subnetName',
                  'uris', 'launchTime']
    ...
    @ex(
        ...
        arguments=ARGS([
            (['appengine'], {'help': 'appengine id', 'action': 'store', 'type': str}),
        ])
    )
    def get(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['farm-name'], {'help': 'name of the farm', 'action': 'store', 'type': int}),
            (['type'], {'help': 'appengine type', 'action': 'store', 'type': str}),
            (['subnet'], {'help': 'appengine subnet id', 'action': 'store', 'type': str}),
            (['sg'], {'help': 'security group id', 'action': 'store', 'type': str}),
            (['-key-name'], {'help': 'ssh key name', 'action': 'store', 'type': str, 'default': None}),
            (['-sharesize'], {'help': 'share size in GB', 'action': 'store', 'type': str, 'default': None}),
            (['-public'], {'help': 'if True appengine has public ip address', 'action': 'store', 'type': bool,
                           'default': False}),
            (['-public-subnet'], {'help': 'public subnet', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def add(self):
    ...
    def delete(self):
        uuid = self.app.pargs.appeng
        uri = '%s/appengineservices/instance/deleteappinstances' % self.baseuri
        res = self.cmp_delete(uri, data={'InstanceId.N': [uuid], 'preserve': True}, timeout=600,
