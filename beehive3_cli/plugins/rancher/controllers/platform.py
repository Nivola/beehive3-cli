...
class RancherPlatformController(BaseController):
    ...
    def pre_command_run(self):
        ...
        if label is None:
            ...
            if len(keys) > 0:
                ...
                raise Exception('No rancher default platform is available for this environment. Select another '
                                'environment')
    ...
    def user_get(self):
        ...
        if oid is not None:
            ...
            headers = ['uuid', 'id', 'name', 'description', 'type', 'state', 'creator', 'created']
            fields = ['uuid', 'id', 'name', 'description', 'type', 'state', 'creatorId', 'created']
    ...
    @ex(
        ...
        arguments=RANCHER_ARGS([
            ...
            (['-chpwd'], {'help': 'change password flag, true or false', 'action': 'store', 'type': bool,
                          'default': False}),
            (['-role'], {'help': 'user role, one among admin, restricted-admin, user, user-base',
    ...
    def cluster_projects_get(self):
        ...
        headers = ['id', 'name', 'description', 'type', 'state', 'namespace', 'created', 'creator']
        fields = ['id', 'name', 'description', 'baseType', 'state', 'namespaceId', 'created', 'creatorId']
        self.app.render(res, key='data', headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=RANCHER_ARGS([
            (['name'], {'help': 'cluster name', 'action': 'store', 'type': str, 'default': None}),
            (['-k8sv'], {'help': 'kubernetes version', 'action': 'store', 'type': str,
                         'default': 'v1.20.11-rancher1-1'}),
            (['-netp'], {'help': 'network provider, one among flannel, calico, canal, weave', 'action': 'store',
                         'type': str, 'default': 'canal'}),
            (['-cloudp'], {'help': 'cloud provider', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    def cluster_get_registration_cmd(self):
        ...
        self.app.render(res, headers=['windows node command'], fields=['windowsNodeCommand'], maxsize=500)
