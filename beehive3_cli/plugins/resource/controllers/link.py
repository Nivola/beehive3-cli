...
class ResourceLinkController(BaseController):
    class Meta:
        ...
        fields = ['id', 'name', 'active', 'details.type', 'details.start_resource', 'details.end_resource',
                  'details.attributes', 'date.creation', 'date.modified']
        headers = ['id', 'name', 'active', 'type', 'start', 'end', 'attributes', 'creation', 'modified']
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['-resource'], {'help': 'start or end resource uuid', 'action': 'store', 'type': str, 'default': None}),
    ...
    def get(self):
        ...
        if oid is not None:
            ...
            self.app.render(res, key='resourcelinks', headers=self._meta.headers, fields=self._meta.fields,
                            maxsize=40)
    ...
    @ex(
        ...
        arguments=ARGS([
            (['name'], {'help': 'resource link name', 'action': 'store', 'type': str}),
            (['type'], {'help': 'resource link type', 'action': 'store', 'type': str, 'default': None}),
            (['start_resource'], {'help': 'start resource uuid', 'action': 'store', 'type': str}),
            (['end_resource'], {'help': 'end resource uuid', 'action': 'store', 'type': str}),
            (['-attributes'], {'help': 'resource link attributes', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def add(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'resource link uuid', 'action': 'store', 'type': str}),
            (['-name'], {'help': 'resource link name', 'action': 'store', 'type': str, 'default': None}),
            (['-type'], {'help': 'resource link type', 'action': 'store', 'type': str, 'default': None}),
            (['-start_resource'], {'help': 'start resource uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-end_resource'], {'help': 'end resource uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-attributes'], {'help': 'resource link attributes', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def update(self):
        ...
        attrib = self.app.pargs.attributes
        data = set_request_params(self.app.pargs, ['name', 'type', 'start_resource', 'end_resource', 'attributes'])
    ...
    @ex(
        ...
        arguments=ARGS([
            (['ids'], {'help': 'comma separated resource link uuids', 'action': 'store', 'type': str}),
        ])
    )
    def patch(self):
