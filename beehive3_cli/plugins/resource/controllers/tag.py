...
class ResourceTagController(BaseController):
    class Meta:
        ...
        fields = ['id', 'name', 'date.creation', 'date.modified', 'resources', 'containers', 'links']
        headers = ['id', 'name', 'creation', 'modified', 'resources', 'containers', 'links']
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'tag uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'tag name', 'action': 'store', 'type': str, 'default': None}),
            (['-resource'], {'help': 'resource', 'action': 'store', 'type': str, 'default': None}),
            (['-container'], {'help': 'container uuid or name', 'action': 'store', 'type': str, 'default': None}),
            (['-type'], {'help': 'tag type', 'action': 'store', 'type': str, 'default': None}),
            (['-objid'], {'help': 'tag authorization id', 'action': 'store', 'type': str, 'default': None}),
            (['-ext_id'], {'help': 'tag physical id', 'action': 'store', 'type': str, 'default': None}),
            (['-parent'], {'help': 'tag parent', 'action': 'store', 'type': str, 'default': None}),
            (['-state'], {'help': 'tag state', 'action': 'store', 'type': str, 'default': None}),
            (['-attributes'], {'help': 'tag attributes', 'action': 'store', 'type': str, 'default': None}),
            (['-tags'], {'help': 'tag tags', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def get(self):
        ...
        if oid is not None:
            ...
            params = ['container', 'type', 'name', 'objid', 'ext_id', 'parent', 'state', 'tags', 'resource']
            mappings = {'name': lambda n: '%' + n + '%'}
            data = self.format_paginated_query(params, mappings=mappings)
            uri = '%s/tags' % self.baseuri
            res = self.cmp_get(uri, data=data)

            self.app.render(res, key='resourcetags', headers=self._meta.headers, fields=self._meta.fields,
                            maxsize=40)
    ...
    @ex(
        ...
        arguments=ARGS([
            (['value'], {'help': 'resource tag value', 'action': 'store', 'type': str}),
        ])
    )
    def add(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-value'], {'help': 'resource tag value', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def update(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['ids'], {'help': 'comma separated resource tag uuids', 'action': 'store', 'type': str}),
        ])
    )
    def patch(self):
