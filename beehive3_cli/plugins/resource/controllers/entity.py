...
class ResourceEntityController(BaseController):
        ...
        for child in resource.get('children', []):
            ...
            if relation is None:
                def create_data():
                    ...
                    yield (Token.Text.Whitespace, ' - ')
                    yield (Token.Literal.String, '%s {%s}' % (child.get('id'), child.get('ext_id')))
    ...
    @ex(
        ...
        arguments=[
            (['entity'], {'help': 'resource entity is or name', 'action': 'store', 'type': str})
        ]
    )
    def errors(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['entity'], {'help': 'resource entity is or name', 'action': 'store', 'type': str}),
            (['-parent'], {'help': 'if True show tree by parent - child', 'action': 'store', 'type': str,
                           'default': None}),
            (['-link'], {'help': 'if True show tree by link relation', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def tree(self):
        oid = self.app.pargs.entity
    ...
    def get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.c('\nlinks', 'underline')
                self.app.render(links.get('resourcelinks'), headers=self._meta.link_headers,
                                fields=self._meta.link_fields)

                uri = '%s/entities/%s/linked' % (self.baseuri, oid)
                linked = self.cmp_get(uri)
                self.c('\nlinked entities', 'underline')
                self.app.render(linked.get('resources'), headers=self._meta.headers, fields=self._meta.fields)
                ...
                self.app.render(res, headers=self._meta.task_headers, fields=self._meta.task_fields, maxsize=80,
                                transform=transform)
            else:
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-type'], {'help': 'entity type', 'action': 'store', 'type': str, 'default': None}),
            (['-subsystem'], {'help': 'entity type subsystem', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def types(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['name'], {'help': 'resource entity name', 'action': 'store', 'type': str}),
            (['-desc'], {'help': 'resource entity description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None}),
            (['container'], {'help': 'resource container uuid', 'action': 'store', 'type': str}),
            (['resclass'], {'help': 'resource entity class', 'action': 'store', 'type': str}),
            (['-ext_id'], {'help': 'resource entity physical id', 'action': 'store', 'type': str, 'default': None}),
            (['-parent'], {'help': 'resource entity parent uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-attribute'], {'help': 'resource entity attributes', 'action': 'store', 'type': str, 'default': None}),
            (['-tags'], {'help': 'resource entity tags', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def add(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-name'], {'help': 'resource entity name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'resource entity description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None}),
            (['-active'], {'help': 'resource entity active', 'action': 'store', 'type': bool, 'default': True}),
            (['-force'], {'help': 'if True force resource metadata update and bypass type specific update',
                          'action': 'store', 'type': str, 'default': None}),
            (['-ext_id'], {'help': 'resource physical id', 'action': 'store', 'type': str, 'default': None}),
            (['-attribute'], {'help': 'resource entity attributes', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def update(self):
        oid = self.app.pargs.id
        data = set_request_params(self.app.pargs, ['force', 'name', 'desc', 'ext_id', 'active', 'attribute'])
        uri = '%s/entities/%s' % (self.baseuri, oid)
    ...
    @ex(
        ...
        arguments=ARGS([
            (['ids'], {'help': 'comma separated resource entity ids', 'action': 'store', 'type': str}),
        ])
    )
    def patch(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-deep'], {'help': 'if false delete only resource record and permissions', 'action': 'store',
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'resource entity id', 'action': 'store', 'type': str}),
            (['-state'], {'help': 'resource state', 'action': 'store', 'type': str, 'default': 'active'}),
        ])
    )
    def state(self):
        oid = self.app.pargs.id
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'resource entity id', 'action': 'store', 'type': str}),
            (['tag'], {'help': 'tag name', 'action': 'store', 'type': str, 'default': 'active'}),
        ])
    )
    def tag_add(self):
        oid = self.app.pargs.id
    ...
    @ex(
        ...
        arguments=PARGS([
            (['id'], {'help': 'resource id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def tag_get(self):
        oid = getattr(self.app.pargs, 'id', None)
