...
class ResourceOrchestratorController(BaseController):
    class Meta:
        ...
        headers = ['id', 'category', 'objdef', 'name', 'active', 'state', 'ping', 'creation', 'modified', 'resources']
        fields = ['id', 'category', '__meta__.definition', 'name', 'active', 'state', 'ping', 'date.creation',
                  'date.modified', 'resources']
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'container uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'container name', 'action': 'store', 'type': str, 'default': None}),
            (['-type'], {'help': 'container type', 'action': 'store', 'type': str, 'default': None}),
            (['-objid'], {'help': 'container authorization id', 'action': 'store', 'type': str, 'default': None}),
            (['-state'], {'help': 'container state', 'action': 'store', 'type': str, 'default': None}),
            (['-attributes'], {'help': 'container attributes', 'action': 'store', 'type': str, 'default': None}),
    ...
    def get(self):
        ...
        if oid is not None:
            ...
            self.app.render(res, key='resourcecontainers', headers=self._meta.headers, fields=self._meta.fields,
    ...
    @ex(
        ...
        arguments=PARGS([
            (['file'], {'help': 'container config as file', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def add(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'entity uuid', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def ping(self):
        oid = self.app.pargs.id
        ...
        res = self.cmp_get(uri)
        self.app.render({'resourcecontainer': oid, 'ping': res['ping']}, headers=['resourcecontainer', 'ping'])
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'entity uuid', 'action': 'store', 'type': str, 'default': None}),
            (['resclass'], {'help': 'entity resclass', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def discover(self):
        oid = self.app.pargs.id
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'container id', 'action': 'store', 'type': str, 'default': None}),
            (['resclass'], {'help': 'entity resclass', 'action': 'store', 'type': str, 'default': None}),
            (['-new'], {'help': 'add new physical entities', 'action': 'store', 'type': str, 'default': 'true'}),
            (['-died'], {'help': 'delete not alive physical entities', 'action': 'store', 'type': str,
                         'default': 'false'}),
            (['-changed'], {'help': 'update physical entities', 'action': 'store', 'type': str, 'default': 'false'}),
            (['-ext_id'], {'help': 'physical entity id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
