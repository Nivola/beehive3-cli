...
class OpenstackController(BaseController):
    class Meta:
        ...
        project_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'level']
        project_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.level']
        network_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'segmentation_id',
                           'external', 'shared', 'provider_type']
        network_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state',
                          'details.segmentation_id', 'details.external', 'details.shared',
                          'details.provider_network_type']
        subnet_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'cidr', 'gateway_ip',
                          'subnet_types', 'enable_dhcp']
        subnet_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.cidr',
                         'details.gateway_ip', 'details.subnet_types', 'details.enable_dhcp']
        port_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'device_owner',
                        'mac_address', 'ip_address']
        port_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.device_owner',
                       'details.mac_address', 'details.fixed_ips.0.ip_address']
        router_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'enable_snat',
                          'external_network', 'external_ip']
        router_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.enable_snat',
                         'details.external_network.name', 'details.external_ips.0.ip_address']
        image_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'size', 'minDisk',
                         'minRam', 'status']
        image_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.size',
                        'details.minDisk', 'details.minRam', 'details.status']
        flavor_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'vcpus', 'ram',
                          'disk']
        flavor_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.vcpus',
                         'details.ram', 'details.disk']
        server_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'runstate',
                          'ip-address', 'hostname', 'cpu', 'ram', 'disk', 'disknum', 'is-template']
        server_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.state',
                         'details.ip_address.0', 'details.hostname', 'details.cpu', 'details.memory', 'details.disk']
        volume_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'status', 'size',
                          'bootable', 'encrypted', 'volume_type']
        volume_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state', 'details.status',
                         'details.size', 'details.bootable', 'details.encrypted', 'details.volume_type']
        stack_headers = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state']
        stack_fields = ['id', 'ext_id', 'name', 'desc', 'parent', 'container', 'state']
        share_headers = ['id', 'ext_id', 'name', 'parent', 'container', 'state', 'share_type',
                         'share_proto', 'size', 'host', 'date']
        share_fields = ['id', 'ext_id', 'name', 'parent', 'container', 'state', 'details.share_type',
                        'details.share_proto', 'details.size', 'details.host', 'date.creation']
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'domain id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def domain_get(self):
    ...
    def project_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='projects', headers=self._meta.project_headers, fields=self._meta.project_fields)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['id'], {'help': 'project id', 'action': 'store', 'type': str, 'default': None}),
            (['type'], {'help': 'quota type', 'action': 'store', 'type': str, 'default': None}),
            (['quota'], {'help': 'quota name', 'action': 'store', 'type': str, 'default': None}),
            (['value'], {'help': 'quota value', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    def network_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='networks', headers=self._meta.network_headers, fields=self._meta.network_fields)
    ...
    def subnet_get(self):
        ...
        if oid is not None:
            ...
            self.app.render(res, key='subnets', headers=self._meta.subnet_headers, fields=self._meta.subnet_fields,
                            transform=transform)
    ...
    def port_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='ports', headers=self._meta.port_headers, fields=self._meta.port_fields)

    @ex(
        help='get security groups',
        description='get security groups',
        arguments=PARGS([
            (['-id'], {'help': 'security group id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def security_group_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                headers = ['id', 'direction', 'protocol', 'ethertype', 'remote_ip_prefix', 'remote_group.name',
                           'remote_group.id', 'port_range_min', 'port_range_max']
                fields = ['id', 'direction', 'protocol', 'ethertype', 'remote_ip_prefix', 'remote_group.name',
                          'remote_group.id', 'port_range_min', 'port_range_max']
                self.app.render(res, details=True)
                self.c('\nrules', 'underline')
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='security_groups', headers=self._meta.headers, fields=self._meta.fields)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['id'], {'help': 'security group id', 'action': 'store', 'type': str, 'default': None}),
            (['rule'], {'help': 'security group rule id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    def router_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                headers = ['id', 'name', 'state', 'network', 'device_owner', 'mac_address', 'subnet',
                           'ip_address']
                fields = ['id', 'name', 'state', 'details.network.id', 'details.device_owner',
                          'details.mac_address', 'details.fixed_ips.0.subnet_id', 'details.fixed_ips.0.ip_address']
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='routers', headers=self._meta.router_headers, fields=self._meta.router_fields)
    ...
    def router_port_del(self):
        ...
        res = self.cmp_delete(uri, data={'router_port': {'subnet_id': subnet}},
    ...
    def server_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.c('\nvolumes', 'underline')
                headers = ['id', 'name', 'format', 'bootable', 'storage', 'mode', 'type', 'size']
                self.app.render(volumes, headers=headers, maxsize=200)
                self.c('\nnetworks', 'underline')
                headers = ['net_id', 'name', 'port_id', 'mac_addr', 'port_state', 'fixed_ips.0.ip_address']
            ...
            res = self.cmp_get(uri, data=data)
            transform = {'desc': lambda x: truncate(x, 20)}
            self.app.render(res, key='servers', headers=self._meta.server_headers, fields=self._meta.server_fields,
                            transform=transform)
    ...
    def volume_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            transform = {'desc': lambda x: truncate(x, 20), 'name': lambda x: truncate(x, 100)}
    ...
    def volume_snapshot_get(self):
        ...
        fields = ['id', 'name', 'status', 'size', 'created_at', 'os-extended-snapshot-attributes:progress']
        self.app.render(res, headers=headers, fields=fields, maxsize=40)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'volumetype id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def volumetype_get(self):
        ...
        if oid is not None:
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='volumetypes', headers=self._meta.headers, fields=self._meta.fields)
    ...
    def stack_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.c('\noutputs', 'underline')
                self.app.render(outputs, headers=['key', 'value', 'desc'],
                                fields=['output_key', 'output_value', 'description'], maxsize=100)

                self.c('\nevents', 'underline')
                uri1 = uri + '/events'
                res = self.cmp_get(uri1).get('stack_events', [])
                headers = ['id', 'name', 'resource_id', 'status', 'status_reason', 'event_time']
                fields = ['id', 'resource_name', 'physical_resource_id', 'resource_status', 'resource_status_reason',
                          'event_time']
                self.app.render(res, headers=headers, fields=fields, maxsize=40)
                ...
                fields = ['physical_resource_id', 'resource_name', 'resource_status', 'resource_type', 'creation_time',
                          'required_by']
                self.app.render(res, headers=headers, fields=fields, maxsize=40)
                ...
                headers = ['id', 'definition', 'name', 'container', 'parent', 'state', 'creation', 'ext_id']
                fields = ['id', '__meta__.definition', 'name', 'container', 'parent', 'state', 'date.creation',
                          'ext_id']
                self.app.render(res, headers=headers, fields=fields, maxsize=40)
            else:
                self.app.render(res, details=True)
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='stacks', headers=self._meta.stack_headers, fields=self._meta.stack_fields)
    ...
    @ex(
        ...
        arguments=ARGS([
            (['orchestrator'], {'help': 'orchestrator id', 'action': 'store', 'type': str, 'default': None}),
            (['template'], {'help': 'template', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def stack_template_functions(self):
        ...
        res = self.cmp_get(uri, data='container=%s&template=%s' % (orchestrator, template))\
            .get('template_functions', {})
    ...
    @ex(
        ...
        arguments=PARGS([
            (['orchestrator'], {'help': 'orchestrator id', 'action': 'store', 'type': str, 'default': None}),
            (['template_uri'], {'help': 'template uri', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    def share_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.app.render(export_locations, headers=['id', 'preferred', 'path', 'share_instance_id'], maxsize=100)

                self.c('\ngrants', 'underline')
                uri = '%s/shares/%s/grant' % (self.baseuri, oid)
                try:
                    res = self.cmp_get(uri).get('share_grant', [])
                    self.app.render(res, headers=['id', 'state', 'access_level', 'access_type', 'access_to'],
                                    maxsize=80)
                except:
                    pass
            else:
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['-desc'], {'help': 'share description', 'action': 'store', 'type': str, 'default': None}),
            (['project'], {'help': 'share project id', 'action': 'store', 'type': str, 'default': None}),
            (['-tags'], {'help': 'comma separated list of tags', 'action': 'store', 'type': str, 'default': None}),
            (['share_proto'], {'help': 'share protocol. A valid value is NFS, CIFS, GlusterFS, HDFS, or CephFS',
                               'action': 'store', 'type': str, 'default': None}),
            (['size'], {'help': 'the share size, in GBs', 'action': 'store', 'type': str, 'default': None}),
            (['share_type'], {'help': 'The share type id.', 'action': 'store', 'type': str, 'default': None}),
            (['-snapshot_id'], {'help': 'The id of the share\'s base snapshot.', 'action': 'store', 'type': str,
                                'default': None}),
            (['-share_group_id'], {'help': 'The id of the share group.', 'action': 'store', 'type': str,
                                   'default': None}),
            (['-network'], {'help': 'id of the network to use', 'action': 'store', 'type': str, 'default': None}),
            (['-subnet'], {'help': 'id of the subnet to use', 'action': 'store', 'type': str, 'default': None}),
            (['-metadata'], {'help': 'One or more metadata key and value pairs as a dictionary of strings.',
                             'action': 'store', 'type': str, 'default': None}),
            (['-availability_zone'], {'help': 'The availability zone.', 'action': 'store', 'type': str,
                                      'default': 'nova'}),
        ])
    )
    def share_add(self):
    ...
    def share_type_get(self):
        ...
        fields = ['id', 'name', 'extra_specs.snapshot_support', 'extra_specs.share_backend_name']
    ...
    @ex(
        ...
        arguments=PARGS([
            (['id'], {'help': 'share id', 'action': 'store', 'type': str, 'default': None}),
            (['access_level'], {'help': 'rw, ro', 'action': 'store', 'type': str, 'default': None}),
            (['access_type'], {'help': 'access type like ip or user', 'action': 'store', 'type': str, 'default': None}),
            (['access_to'], {'help': 'access to like 10.102.185.0/24 or admin/user', 'action': 'store', 'type': str,
                             'default': None}),
        ])
    )
