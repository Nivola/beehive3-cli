...
class VirshPlatformController(BaseController):
    ...
    def hosts(self):
        ...
        headers = ['hostname', 'hypervisor', 'arch', 'cpu', 'ram', 'ip_address', 'vm-running']
        fields = ['hostname', 'hypervisor.version', 'info.0', 'info.2', 'info.1', 'ip_address', 'vm_running']
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-status'], {'help': 'domain status can be: 1 - ACTIVE, 2 - INACTIVE, 4 - PERSISTENT, 8 - TRANSIENT, '
                                   '16 - RUNNING, 32 - PAUSED, 64 - SHUTOFF, 128 - OTHER', 'action': 'store',
                           'type': int, 'default': 1}),
            (['-id'], {'help': 'domain name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def domain_get(self):
        ...
        if domain is None:
            ...
            headers = ['uuid', 'name', 'ops-name', 'ops-project', 'cpu', 'ram', 'ip-address', 'active', 'state', 'time',
                       'quest_agent']
            fields = ['uuid', 'name', 'metadata.nova:instance.nova:name',
                      'metadata.nova:instance.nova:owner.nova:project.uuid', 'cpu.sockets', 'memory.#text',
                      'ifaces.eth0.addrs.0.addr', 'active', 'state', 'time', 'quest_agent']
            self.app.render(resp, headers=headers, fields=fields, maxsize=200)
        else:
            ...
            if self.is_output_text():
                ...
                headers = {
                    'disk': ['device', 'type', 'source.file', 'target.dev', 'target.bus'],
                    'controller': ['type', 'index', 'model'],
                    'interface': ['type', 'mac.address', 'source.bridge', 'virtualport.type', 'target.dev', 'mtu.size'],
                    'serial': ['type', 'log.file', 'target.type', 'target.port'],
                    'console': ['type', 'log.file', 'target.type', 'target.port'],
                    'channel': ['type', 'source.mode', 'source.path', 'target.type', 'target.name', 'address.type'],
                ...
                self.app.render(file_system_info, headers=['name', 'mountpoint', 'type', 'disk.0.bus-type', 'size',
                                                           'usage'])
            else:
                self.app.render(resp, details=True)
    ...
    def domain_ping(self):
        ...
        if domain is None:
            ...
            fields = ['uuid', 'name', 'metadata.nova:instance.nova:name', 'ifaces.eth0.addrs.0.addr', 'ping']
            self.app.render(resp, headers=headers, fields=fields, maxsize=200)
        else:
            domain = server.get_domain(name=domain)
    ...
    def domain_usage(self):
        ...
        if domain is None:
            ...
            def transform_ifaces(data):
                resp = []
                for item in data:
                    resp.append('%s: rx %s,tx %s' % (item['name'], dict_get(item, 'statistics.rx-bytes', default=''),
                                dict_get(item, 'statistics.tx-bytes', default='')))
                return ', '.join(resp)

            transform = {'file_system': transform_file_system, 'ifaces': transform_ifaces}

            headers = ['uuid', 'name', 'ops-name', 'ip-address', 'ping', 'file_system', 'ifaces']
            fields = ['uuid', 'name', 'metadata.nova:instance.nova:name', 'ifaces.1.ip-addresses.0.ip-address', 'ping',
                      'file_system', 'ifaces']
        ...
        if domain is None:
            ...
            for item in data:
                ...
                row = {
                    ...
                    'vcpu': {'count': vcpu_count, 'time': [item.get('vcpu.%s.time' % i) for i in range(vcpu_count)]},
                    'block': blocks,
                    'memory': {
            ...
            fields = ['name', 'vcpu.count', 'memory.usable', 'net.tx', 'net.rx', 'block']
            self.app.render(resp, headers=headers, fields=fields, maxsize=200)
        else:
            domain = server.get_domain(name=domain)
            self.app.render(domain.ext_info(), details=True)
    ...
    def servers(self):
        ...
        headers = ['ops_id', 'ops_name', 'ops_status', 'virt_id', 'virt_name', 'virt_status', 'is_openstack', 'is_virt']
