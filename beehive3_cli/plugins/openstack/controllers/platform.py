...
class OpenstackPlatformController(BaseController):
    ...
    def pre_command_run(self):
        ...
        self.client.authorize(conf.get('user'), conf.get('pwd'), project=project, domain=conf.get('domain'),
                              key=self.key)
    ...
    def __get_orchestartors(self, instances, vip=True):
        ...
        for instance in instances:
            ...
            for host in hosts:
                uri = '%s://%s:%s%s' % (conf.get('proto'), host, conf.get('port'), conf.get('path'))
    ...
    def __get_client(self, conf):
        ...
        client.authorize(conf.get('user'), str(conf.get('pwd')), project=conf.get('project'),
                         domain=conf.get('domain'), key=self.key)
        return client
    ...
    def run_cmd(self, func, configs):
        ...
        try:
            ...
            for config in configs:
                ...
                res = func(config)
                elapsed = time() - start
                resp.append({'instance': config['instance'],
                             'host': config['host'],
                             'elapsed': elapsed, 'response': res})
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-vip'], {'help': 'if true check only the vip', 'action': 'store', 'type': str, 'default': 'true'}),
        ])
    )
    def ultra_ping(self):
        instances = None
    ...
    def sys_api_get(self):
        ...
        for r in res:
            r['component'] = 'manila'
        resp.extend(res)
        self.app.render(resp, headers=['component', 'id', 'version', 'min_version', 'status', 'updated'])
    ...
    def sys_compute_zone_get(self):
        ...
        for item in res:
            ...
            if hosts is None:
                hosts = {}
            resp.append({'state': item['zoneState']['available'],
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['host'], {'help': 'host id', 'action': 'store', 'type': str})
        ])
    )
    ...
    def sys_compute_host_aggregate_get(self):
        ...
        self.app.render(res, headers=['id', 'name', 'availability_zone', 'hosts', 'created_at'], maxsize=200)
    ...
    def sys_compute_hypervisor_get(self):
        ...
        headers = ['id', 'hypervisor_hostname', 'host_ip', 'status', 'state', 'vcpus', 'vcpus_used',
                   'memory_mb', 'free_ram_mb', 'local_gb', 'local_gb_used', 'free_disk_gb', 'current_workload',
                   'running_vms']
        self.app.render(res, headers=headers, maxsize=200)
    ...
    def sys_compute_hypervisor_stats(self):
        ...
        headers = ['count', 'vcpus_used', 'local_gb_used', 'memory_mb', 'current_workload', 'vcpus', 'running_vms', 
                   'free_disk_gb', 'disk_available_least', 'local_gb', 'free_ram_mb', 'memory_mb_used']
        self.app.render(res, headers=headers, maxsize=200)
    ...
    def sys_compute_hypervisor_servers(self):
        ...
        headers = ['id', 'hypervisor_hostname', 'host_ip', 'status', 'state', 'running_vms', 'vms']
    ...
    def sys_storage_host_get(self):
        ...
        headers = ['zone', 'service', 'service-status', 'service-state', 'host_name', 'last-update']
    ...
    def sys_storage_backend_storage_pool_get(self):
        ...
        headers = ['name', 'qos', 'protocol', 'driver_version', 'reserved_%', 'backend_name', 'free_gb',
                   'total_gb', 'updated']
        fields = ['name', 'capabilities.QoS_support', 'capabilities.storage_protocol',
                  'capabilities.driver_version', 'capabilities.reserved_percentage',
                  'capabilities.volume_backend_name', 'capabilities.free_capacity_gb',
                  'capabilities.total_capacity_gb', 'capabilities.updated']
    ...
    def sys_network_agent_get(self):
        ...
        headers = ['id', 'host', 'availability_zone', 'binary', 'agent_type', 'alive', 'started_at']
        self.app.render(res, headers=headers, maxsize=200)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'project id', 'action': 'store', 'type': str, 'default': None}),
            (['quota_type'], {'help': 'project quota type. can be compute, block, network, share',
                              'action': 'store', 'type': str, 'default': None}),
            (['quota'], {'help': 'project quota name', 'action': 'store', 'type': str, 'default': None}),
            (['value'], {'help': 'project quota value', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def project_quota_set(self):
        ...
        self.app.render({'msg': 'update quota %s %s with value %s' % (quota_type, quota, value)}, maxsize=200)

    @ex(
        help='add openstack project',
        description='add openstack project',
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'project name', 'action': 'store', 'type': str, 'default': None}),
            (['domain'], {'help': 'project domain id', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'project description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None}),
            (['-parent'], {'help': 'parent project id', 'action': 'store', 'type': str, 'default': None}),
            (['-enabled'], {'help': 'enabled status', 'action': 'store', 'type': str, 'default': 'true'}),
        ])
    )
    def project_add(self):
        ...
        res = self.client.project.create(name, domain, is_domain=False, parent_id=parent_id, description=description,
                                         enabled=enabled)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-desc'], {'help': 'project description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None}),
            (['-domain'], {'help': 'parent domain id', 'action': 'store', 'type': str, 'default': None}),
            (['-parent'], {'help': 'parent project id', 'action': 'store', 'type': str, 'default': None}),
            (['-enabled'], {'help': 'enabled status', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def project_update(self):
        ...
        parent_id = self.app.pargs.parent
        self.client.project.update(oid, name=name, domain=domain, enabled=enabled, description=description,
                                   parent_id=parent_id)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'network id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def network_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                headers = ['id', 'tenant_id', 'name', 'cidr', 'pools', 'gateway_ip', 'enable_dhcp', 'service_types']
                fields = ['id', 'tenant_id', 'name', 'cidr', 'allocation_pools', 'gateway_ip', 'enable_dhcp',
                          'service_types']
                ...
                headers = ['id', 'tenant_id', 'name', 'ip_address', 'mac_address', 'status', 'device_owner',
                           'security_groups']
                fields = ['id', 'tenant_id', 'name', 'fixed_ips.0.ip_address', 'mac_address', 'status', 'device_owner',
                          'security_groups']
            ...
            headers = ['id', 'tenant_id', 'name', 'segmentation_id', 'external', 'shared', 'type', 'mtu']
            fields = ['id', 'tenant_id', 'name', 'provider:segmentation_id', 'router:external', 'shared',
                      'provider:network_type', 'mtu']
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-physical_network'], {'help': 'physical network', 'action': 'store', 'type': str, 'default': None}),
            (['-segmentation_id'], {'help': 'segmentation id', 'action': 'store', 'type': str, 'default': None}),
            (['-type'], {'help': 'network type. Can be flat, vlan, vxlan, or gre', 'action': 'store',
                                 'type': str, 'default': 'vlan'}),
            (['-mt'], {'help': 'the maximum transmission unit MTU', 'action': 'store', 'type': int, 'default': 1500}),
        ])
    )
    def network_add(self):
        ...
        self.client.network.create(name, tenant_id, physical_network, segmentation_id=segmentation_id,
                                   network_type=network_type, mtu=mtu)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'network id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'network name', 'action': 'store', 'type': str, 'default': None}),
            (['-shared'], {'help': 'network shared', 'action': 'store', 'type': str, 'default': None}),
            (['-mtu'], {'help': 'network mtu', 'action': 'store', 'type': int, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'subnet id', 'action': 'store', 'type': str, 'default': None}),
            (['-project'], {'help': 'project id', 'action': 'store', 'type': str, 'default': None}),
    ...
    def subnet_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                end = ipaddress.IPv4Address(dict_get(res, 'allocation_pools.0.end'))
                start = ipaddress.IPv4Address(dict_get(res, 'allocation_pools.0.start'))
                num = 1
                while start != end:
                    start += 1
                    num += 1
            ...
            headers = ['id', 'tenant_id', 'name', 'network_id', 'cidr', 'enable_dhcp', 'service_types']
            fields = ['id', 'tenant_id', 'name', 'network_id', 'cidr', 'enable_dhcp', 'service_types']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['network'], {'help': 'parent network id', 'action': 'store', 'type': str, 'default': None}),
            (['-gateway'], {'help': 'gateway ip', 'action': 'store', 'type': str, 'default': None}),
            (['cidr'], {'help': 'subnet cidr', 'action': 'store', 'type': str, 'default': None}),
            (['-enable-dhcp'], {'help': 'enable dhcp', 'action': 'store', 'type': str, 'default': 'true'}),
            (['dns'], {'help': 'comma separated subnet dns', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def subnet_add(self):
        name = self.app.pargs.name
        ...
        self.client.network.subnet.create(name, network_id, tenant_id, gateway_ip, cidr, allocation_pools=None,
                                          enable_dhcp=enable_dhcp, host_routes=None,
                                          dns_nameservers=dns, service_types=None)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'subnet id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'subnet name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'subnet description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'port id', 'action': 'store', 'type': str, 'default': None}),
            (['-project'], {'help': 'project id', 'action': 'store', 'type': str, 'default': None}),
            (['-network'], {'help': 'network id', 'action': 'store', 'type': str, 'default': None}),
            (['-status'], {'help': 'the port status. Value is ACTIVE or DOWN', 'action': 'store', 'type': str,
                           'default': None}),
            (['-device'], {'help': 'the id of the device that uses this port', 'action': 'store', 'type': str,
                           'default': None}),
            (['-device_owner'], {'help': 'the entity type that uses this port. For example: - compute:nova (server '
                                         'instance), network:dhcp (DHCP agent) - network:router_interface (router '
                                         'interface).', 'action': 'store', 'type': str, 'default': None}),
            (['-security_group'], {'help': 'the id of any attached security groups', 'action': 'store', 'type': str,
                                   'default': None}),
    ...
    def port_get(self):
        ...
        if oid is not None:
            ...
            # subnet_id = self.app.pargs.subnet
            # ip_address = self.app.pargs.ip_address
            res = self.client.network.port.list(tenant=tenant, network=network, status=status, device_id=device_id,
                                                device_owner=device_owner, security_group=security_group)
            headers = ['id', 'tenant_id', 'name', 'ip_address', 'mac_address', 'status', 'device_owner',
                       'security_groups']
            fields = ['id', 'tenant_id', 'name', 'fixed_ips.0.ip_address', 'mac_address', 'status', 'device_owner',
                      'security_groups']
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['network'], {'help': 'network id', 'action': 'store', 'type': str, 'default': None}),
            (['-subnet'], {'help': 'subnet id', 'action': 'store', 'type': str, 'default': None}),
            (['-tenant'], {'help': 'tenant id', 'action': 'store', 'type': str, 'default': None}),
            (['-ipaddress'], {'help': 'ip addresses associated to subnet', 'action': 'store', 'type': str,
                              'default': None}),
            (['-allowed-ipaddress'], {'help': 'allowed ip address', 'action': 'store', 'type': str, 'default': None}),
            (['-allowed-macaddress'], {'help': 'allowed ip macaddress', 'action': 'store', 'type': str,
                                       'default': None}),
            (['-host'], {'help': 'host id', 'action': 'store', 'type': str, 'default': None}),
            (['-device_owner'], {'help': 'device owner', 'action': 'store', 'type': str, 'default': None}),
            (['-security_groups'], {'help': 'One or more security group id.', 'action': 'store', 'type': str,
                                    'default': None}),
        ])
    )
    def port_add(self):
        ...
        if tenant_id is None:
            tenant_id = network['tenant_id']
        res = self.client.network.port.create(name, network_id, fixed_ips, host_id=host_id, tenant_id=tenant_id,
                                              profile=None, vnic_type=None, device_owner=device_owner, device_id=None,
                                              security_groups=security_groups, mac_address=None,
                                              allowed_address_pairs=allowed_address_pairs)
    ...
    def port_add_batch(self):
        ...
        for i in range(2, 171):
            ...
            if ip_address not in allocated_port_ips:
                port = self.client.network.port.create(name, network_id, fixed_ips, tenant_id=tenant_id, profile=None,
                                                       vnic_type=None, device_owner=device_owner,
                                                       allowed_address_pairs=allowed_address_pairs)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'port id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'port name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'port description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None}),
            (['-sgs'], {'help': 'port security groups comma separated', 'action': 'store', 'type': str,
                        'default': None}),
            (['-allowed_ips'], {'help': 'a comma separated list of allowed ip address', 'action': 'store', 'type': str,
                                'default': None}),
            (['-port_security_enabled'], {'help': 'enable or disable port security', 'action': 'store', 'type': str,
                                          'default': None}),
        ])
    )
    def port_update(self):
        ...
        res = self.client.network.port.update(oid, name, desc, security_groups=sgs,
                                              allowed_address_pairs=allowed_address_pairs,
    ...
    def floatingip_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'tenant_id', 'status', 'floating_ip_address', 'fixed_ip_address']
            fields = ['id', 'tenant_id', 'status', 'floating_ip_address', 'fixed_ip_address']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'router id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def router_get(self):
        oid = getattr(self.app.pargs, 'id', None)
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                headers = ['id', 'name', 'ip_address', 'mac_address', 'status', 'device_owner', 'security_groups']
                fields = ['id', 'name', 'fixed_ips.0.ip_address', 'mac_address', 'status', 'device_owner',
                          'security_groups']
                self.app.render(ports, headers=headers, fields=fields)
            else:
                self.app.render(res, details=True)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['tenant'], {'help': 'parent project id', 'action': 'store', 'type': str, 'default': None}),
            (['-network'], {'help': 'physical router', 'action': 'store', 'type': str, 'default': None}),
            (['-ext-subnet'], {'help': 'external subnet', 'action': 'store', 'type': str, 'default': None}),
            (['-ext-ip'], {'help': 'external ip address', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def router_add(self):
        name = self.app.pargs.name
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'router id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'router name', 'action': 'store', 'type': str, 'default': None}),
            (['-network'], {'help': 'physical router', 'action': 'store', 'type': str, 'default': None}),
            (['-ext-subnet'], {'help': 'external subnet', 'action': 'store', 'type': str, 'default': None}),
            (['-ext-ip'], {'help': 'external ip address', 'action': 'store', 'type': str, 'default': None}),
            (['-routes'], {'help': 'routes like dest:nexthop,dest:nexthop', 'action': 'store', 'type': str,
                           'default': None}),
        ])
    )
    def router_update(self):
        ...
        self.client.network.router.update(oid, name=name, network=network, external_ips=external_ips,
    ...
    def router_rewrite_routes(self):
        ...
        router_routes = []
        for route in routes:
            router_routes.append({'nexthop': route.get('nexthop'), 'destination': route.get('destination')})
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'router id', 'action': 'store', 'type': str, 'default': None}),
            (['subnet'], {'help': 'subnet id', 'action': 'store', 'type': str, 'default': None}),
            (['-ip'], {'help': 'intarface ip', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    def security_group_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.app.render(res, details=True)
                self.c('\nrules', 'underline')
                self.app.render(rules, headers=['id', 'direction', 'ethertype', 'remote_group_id', 'remote_ip_prefix',
                                                'protocol', 'port_range_min', 'port_range_max', 'created_at',
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'security_group name', 'action': 'store', 'type': str}),
            (['project'], {'help': 'parent project id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'security_group id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'security_group name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'security_group description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'security group rule id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'security group rule id', 'action': 'store', 'type': str, 'default': None}),
            (['-direction'], {'help': 'direction. Can be ingress or egress', 'action': 'store', 'type': str,
                              'default': 'ingress'}),
            (['-ethertype'], {'help': 'Must be IPv4 or IPv6', 'action': 'store', 'type': str,
                              'default': 'IPv4'}),
            (['-port_min'], {'help': 'port range min', 'action': 'store', 'type': str, 'default': None}),
            (['-port_max'], {'help': 'port range max', 'action': 'store', 'type': str, 'default': None}),
            (['-protocol'], {'help': 'protocol', 'action': 'store', 'type': str, 'default': None}),
            (['-remote_ip'], {'help': 'remote ip', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def security_group_rule_add(self):
        ...
        self.client.network.security_group.create_rule(oid, direction, ethertype=ethertype, port_range_min=port_min,
                                                       port_range_max=port_max, protocol=protocol, remote_group_id=None,
    ...
    def image_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'name', 'owner', 'status', 'disk_format', 'created_at', 'min_disk', 'min_Ram', 'size',
                       'hw_qemu_guest_agent', 'img_config_drive']
            fields = ['id', 'name', 'owner', 'status', 'disk_format', 'created_at', 'min_disk', 'min_ram', 'size',
                      'hw_qemu_guest_agent', 'img_config_drive']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-disk_format'], {'help': 'disk format. Ex.: ami, ari, aki, vhd, vhdx, vmdk, raw, qcow2, vdi, ploop, iso',
                                'action': 'store', 'type': str, 'default': 'qcow2'}),
            (['-min_disk'], {'help': 'amount of disk space in GB that is required to boot the image.',
                             'action': 'store', 'type': str, 'default': None}),
            (['-min_ram'], {'help': 'amount of RAM in MB that is required to boot the image',
                                    'action': 'store', 'type': str, 'default': None})
        ])
    )
    def image_add(self):
        ...
        self.client.image.create(name, disk_format=disk_format, min_disk=min_disk, min_ram=min_ram)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'image id', 'action': 'store', 'type': str, 'default': None}),
            (['name'], {'help': 'image file name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'image task id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    def flavor_get(self):
        ...
        if oid is not None:
            ...
            for item in res:
                ...
                item['aggregato'] = dict_get(aggregate_instance_extra_specs, 'aggregate_instance_extra_specs:tipo')
            headers = ['id', 'name', 'ram', 'vcpus', 'swap', 'is_public', 'rxtx_factor', 'disk',
                       'ephemeral', 'disabled', 'aggregato']
            fields = ['id', 'name', 'ram', 'vcpus', 'swap', 'os-flavor-access:is_public', 'rxtx_factor', 'disk',
                      'OS-FLV-EXT-DATA:ephemeral', 'OS-FLV-DISABLED:disabled', 'aggregato']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['vcpu'], {'help': 'vcpu', 'action': 'store', 'type': str, 'default': None}),
            (['ram'], {'help': 'ram', 'action': 'store', 'type': str, 'default': None}),
            (['disk'], {'help': 'disk', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def flavor_add(self):
        name = self.app.pargs.name
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'flavor id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'flavor name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'flavor description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'flavor id', 'action': 'store', 'type': str, 'default': None}),
            (['-spec'], {'help': 'flavor extra spec key', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'flavor id', 'action': 'store', 'type': str, 'default': None}),
            (['spec'], {'help': 'flavor extra spec keys. Syntax k1:v1,k2:v2', 'action': 'store', 'type': str,
                        'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'flavor id', 'action': 'store', 'type': str, 'default': None}),
            (['spec'], {'help': 'flavor extra spec key. Syntax k1:v1', 'action': 'store', 'type': str,
                        'default': None}),
            (['spec_value'], {'help': 'flavor extra spec key', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'aggregate id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def aggregate_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'uuid', 'name', 'availability_zone', 'hosts', 'metadata', 'created_at', 'updated_at',
                       'deleted_at']
            fields = ['id', 'uuid', 'name', 'availability_zone', 'hosts', 'metadata', 'created_at', 'updated_at',
                      'deleted_at']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['availability_zone'], {'help': 'availability zone', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'aggregate id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'aggregate name', 'action': 'store', 'type': str, 'default': None}),
            (['-availability_zone'], {'help': 'aggregate availability zone', 'action': 'store', 'type': str,
                                      'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'aggregate id', 'action': 'store', 'type': str, 'default': None}),
            (['host'], {'help': 'host_id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'aggregate id', 'action': 'store', 'type': str, 'default': None}),
            (['metadata'], {'help': 'key:value,key:value', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'server group id', 'action': 'store', 'type': str, 'default': None}),
            (['-size'], {'help': 'number of item to return', 'action': 'store', 'type': int, 'default': 20}),
            (['-page'], {'help': 'page to return', 'action': 'store', 'type': int, 'default': 0}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'server group name', 'action': 'store', 'type': str}),
            (['-policy'], {'help': 'one policy name to associate with the server group. Policy names are:'
                                   ...
                                   'be scheduled instead of resulting in a build failure', 'action': 'store',
                           'type': str, 'default': 'soft-anti-affinity'}),
        ])
    )
    def server_group_add(self):
        name = self.app.pargs.name
        policy = self.app.pargs.policy
        if policy not in ['anti-affinity', 'affinity', 'soft-anti-affinity', 'soft-affinity']:
            raise Exception('only anti-affinity, affinity, soft-anti-affinity and soft-affinity are supported as '
                            'policy')
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'server group id', 'action': 'store', 'type': str, 'default': None}),
            (['server'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def server_group_member_add(self):
        ...
        def statements(conn):
            ...
            if len(group) == 1:
                group_id = group[0][2]
                conn.execute('INSERT INTO `instance_group_member`(created_at, instance_uuid, group_id) '
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-host'], {'help': 'compute node name', 'action': 'store', 'type': str, 'default': None}),
            (['-status'], {'help': 'server status', 'action': 'store', 'type': str, 'default': None}),
            (['-updated_at'], {'help': 'Filter the server list result by a date and time stamp when the instance was '
    ...
    def server_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                res['flavor']['name'] = self.client.flavor.get(flavor_id).get('name')
                self.app.render(res, details=True)

                # get volumes
                volumes = []
                for item in server_volumes:
                    volume = self.client.volume_v3.get(item['id'])
                    volumes.append(volume)
                headers = ['id', 'name', 'os-vol-tenant-attr:tenant_id', 'size', 'status', 'type', 'bootable']
                fields = ['id', 'name', 'os-vol-tenant-attr:tenant_id', 'size', 'status', 'volume_type', 'bootable']
                self.c('\nvolumes', 'underline')
            ...
            host = self.app.pargs.host
            status = self.app.pargs.status
            updated_at = self.app.pargs.updated_at
            res = self.client.server.list(detail=True, host=host, status=status, updated_at=updated_at)
            projects = {p['id']: p for p in self.client.project.list()}
            for i in res:
                server_addrs = []
                for vlan, addrs in i.get('addresses').items():
                    server_addrs.append(dict_get(addrs, '0.addr'))
                i['addr'] = ','.join(server_addrs)

                project = projects.get(i['tenant_id'])
                i['tenant_name'] = ''
                if project is not None:
                    i['tenant_name'] = project.get('description')

            headers = ['id', 'tenant', 'name', 'instance_name', 'status', 'power_state', 'host', 'addr', 'updated']
            fields = ['id', 'tenant_name', 'name', 'OS-EXT-SRV-ATTR:instance_name', 'status', 'OS-EXT-STS:power_state',
                      'OS-EXT-SRV-ATTR:host', 'addr', 'updated']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-host'], {'help': 'node name', 'action': 'store', 'type': str, 'default': None}),
            (['-loop'], {'help': 'if value > 1 enable query loop', 'action': 'store', 'type': int, 'default': 1}),
        ])
    )
    def host_status(self):
        ...
        for host_info in res:
            ...
            for i in range(loop):
                ...
                resp = {'active': 0, 'shutoff': 0, 'error': 0, 'no-state': 0, 'rebuild': 0}
                for i in res:
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['project'], {'help': 'parent project id', 'action': 'store', 'type': str, 'default': None}),
            (['physical_server'], {'help': 'physical server', 'action': 'store', 'type': str, 'default': None}),
            (['-segmentation_id'], {'help': 'segmentation id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def server_add(self):
        name = self.app.pargs.name
        tenant_id = self.app.pargs.tenant
        physical_server = self.app.pargs.physical_server
        segmentation_id = self.app.pargs.segmentation_id
        self.client.server.create(name, tenant_id, physical_server, segmentation_id=segmentation_id)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-desc'], {'help': 'server description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None})
        ])
    )
    def server_update(self):
        ...
        name = self.app.pargs.name
        desc = self.app.pargs.desc
        res = self.client.server.update(oid, name, desc)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
            (['state'], {'help': 'server state', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
            (['-port'], {'help': 'port id', 'action': 'store', 'type': str, 'default': None}),
            (['-net'], {'help': 'network id', 'action': 'store', 'type': str, 'default': None}),
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
            (['sg'], {'help': 'security group id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
            (['-action'], {'help': 'action id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def server_action_get(self):
        ...
        if action is not None:
            ...
            headers = ['action-id', 'action', 'user-id', 'project-id', 'instance-id', 'start-time', 'message']
            fields = ['request_id', 'action', 'user_id', 'project_id', 'instance_uuid', 'start_time', 'message']
            self.app.render(res, headers=headers, fields=fields)
    ...
    def server_migration_list(self):
        ...
        self.app.render(res, headers=['id', 'uuid', 'instance_uuid', 'source_node', 'dest_node', 'status',
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
            (['migration_id'], {'help': 'server migration id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'server id', 'action': 'store', 'type': str, 'default': None}),
            (['-host'], {'help': 'host id', 'action': 'store', 'type': str, 'default': None}),
            (['-live'], {'help': 'if true enable live migration', 'action': 'store', 'type': str, 'default': 'true'}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['from_host'], {'help': 'starting host', 'action': 'store', 'type': str, 'default': None}),
            (['-to_host'], {'help': 'destination host', 'action': 'store', 'type': str, 'default': None}),
            (['-live'], {'help': 'if true enable live migration', 'action': 'store', 'type': str, 'default': 'true'}),
        ])
    )
    ...
    def stack_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.c('\noutputs', 'underline')
                self.app.render(outputs, headers=['key', 'value', 'desc', 'error'],
                                fields=['output_key', 'output_value', 'description', 'output_error'], maxsize=50)
            ...
            headers = ['id', 'project', 'stack_name', 'stack_owner', 'stack_status', 'creation_time']
            fields = ['id', 'project', 'stack_name', 'stack_owner', 'stack_status', 'creation_time']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['project'], {'help': 'parent project id', 'action': 'store', 'type': str, 'default': None}),
            (['physical_stack'], {'help': 'physical stack', 'action': 'store', 'type': str, 'default': None}),
            (['-segmentation_id'], {'help': 'segmentation id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def stack_add(self):
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'stack id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'stack name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'stack description', 'action': 'store', 'action': StringAction, 'type': str,
                         'nargs': '+', 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'stack id', 'action': 'store', 'type': str, 'default': None}),
            (['-resource'], {'help': 'stack resource name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def stack_resource_get(self):
        ...
        if resource is None:
            ...
            fields = ['physical_resource_id', 'resource_name', 'resource_status', 'resource_type',
                      'creation_time', 'required_by']
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'share type id', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'share type description', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def share_type_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'name', 'description', 'extra_specs', 'required_extra_specs']
            fields = ['id', 'name', 'description', 'extra_specs', 'required_extra_specs']
            ...
            transform = {'extra_specs': extra_specs_func, 'required_extra_specs': extra_specs_func}
    ...
    def share_message_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'action_id', 'resource_type', 'resource_id', 'message_level', 'created_at', 'user_message']
            fields = ['id', 'action_id', 'resource_type', 'resource_id', 'message_level', 'created_at', 'user_message']
    ...
    def share_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.app.render(grants, headers=['id', 'access_type', 'access_level', 'state', 'access_to'])
            else:
                self.app.render(res, details=True)
        else:
            params = {}
            res = self.client.manila.share.list(details=True, **params)
            headers = ['id', 'name', 'project_id', 'size', 'created_at', 'share_type', 'share_proto', 'status',
                       'is_public']
            fields = ['id', 'name', 'project_id', 'size', 'created_at', 'share_type', 'share_proto', 'status',
                      'is_public']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'share name', 'action': 'store', 'type': str, 'default': None}),
            (['size'], {'help': 'share size in GB', 'action': 'store', 'type': int, 'default': 1}),
            (['proto'], {'help': 'share protocol (NFS, CIFS, GlusterFS, HDFS, or CephFS. CephFS)', 'action': 'store',
                         'type': str, 'default': None}),
            (['type'], {'help': 'share type id', 'action': 'store', 'type': str, 'default': None}),
            (['-snapshot'], {'help': 'share snapshot id', 'action': 'store', 'type': str, 'default': None}),
            (['-group'], {'help': 'share group id', 'action': 'store', 'type': str, 'default': None}),
            (['-network'], {'help': 'the id of a share network where the share server exists or will be created. '
                                    'If is None and you provide a snapshot_id, the network value from the snapshot '
                                    'is used', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def share_add(self):
        name = self.app.pargs.name
        ...
        res = self.client.manila.share.create(proto, size, name=name, description=name, share_type=share_type,
                                              is_public=False, availability_zone='nova', snapshot_id=snapshot,
                                              share_group_id=group, share_network_id=network)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'share id', 'action': 'store', 'type': str, 'default': None}),
            (['-force'], {'help': 'if true force delete', 'action': 'store', 'type': bool, 'default': True}),
        ])
    )
    def share_del(self):
        oid = self.app.pargs.id
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'share id', 'action': 'store', 'type': str, 'default': None}),
            (['size'], {'help': 'new size of the share, in GBs.', 'action': 'store', 'type': int, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'share network id', 'action': 'store', 'type': str, 'default': None}),
            (['-network'], {'help': 'neutron network id', 'action': 'store', 'type': str, 'default': None}),
            (['-subnet'], {'help': 'neutron subnet id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def share_network_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'name', 'project_id', 'created_at', 'neutron_net_id', 'neutron_subnet_id']
            fields = ['id', 'name', 'project_id', 'created_at', 'neutron_net_id', 'neutron_subnet_id']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'share network name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'share network description', 'action': 'store', 'type': str, 'default': None}),
            (['network'], {'help': 'The UUID of a neutron network when setting up or updating a share network subnet '
                                   'with neutron. Specify both a neutron network and a neutron subnet that belongs '
                                   'to that neutron network.', 'action': 'store', 'type': str, 'default': None}),
            (['subnet'], {'help': 'The UUID of the neutron subnet when setting up or updating a share network subnet '
                                  'with neutron. Specify both a neutron network and a neutron subnet that belongs to '
                                  'that neutron network.', 'action': 'store', 'type': str, 'default': None}),
            (['-availability_zone'], {'help': 'The UUID or name of an availability zone for the share network subnet.',
                                      'action': 'store', 'type': str, 'default': 'nova'})
        ])
    )
    def share_network_add(self):
        ...
        if desc is None:
            desc = name
        self.client.manila.network.create(name=name, description=desc, neutron_net_id=network,
                                          neutron_subnet_id=subnet, availability_zone=availability_zone)
    ...
    def share_network_update(self):
        ...
        if desc is None:
            desc = name
        self.client.manila.network.create(oid, name=name, description=desc, neutron_net_id=network,
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'share network id', 'action': 'store', 'type': str, 'default': None}),
            (['service'], {'help': 'security service id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'share server id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def share_server_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'project_id', 'host', 'share_network_id', 'share_network_name', 'updated_at', 'status']
            fields = ['id', 'project_id', 'host', 'share_network_id', 'share_network_name', 'updated_at', 'status']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'share id', 'action': 'store', 'type': str, 'default': None}),
            (['-level'], {'help': 'the access level to the share. rw: Read and write (RW) access. ro: Read-only (RO) '
                                  'access.', 'action': 'store', 'type': str, 'default': 'rw'}),
            (['-type'], {'help': 'the access rule type. ip: Authenticates an instance through its IP address. cert: '
                                 'Authenticates an instance through a TLS certificate. user: Authenticates by a user '
                                 'or group name.', 'action': 'store', 'type': str, 'default': 'ip'}),
            (['-to'], {'help': 'the value that defines the access. ip: A valid format is XX.XX.XX.XX or XX.XX.XX.XX/XX.'
                               ...
                               'characters long.', 'action': 'store', 'type': str, 'default': '0.0.0.0/0'}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'share id', 'action': 'store', 'type': str, 'default': None}),
            (['access_id'], {'help': 'grant id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'share id', 'action': 'store', 'type': str, 'default': None}),
            (['-status'], {'help': 'the share access status, which is new, error, active', 'action': 'store',
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'share id', 'action': 'store', 'type': str, 'default': None}),
            (['snapshot_id'], {'help': 'the share snapshot id', 'action': 'store', 'type':  str, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'volume message id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-hostname'], {'help': 'backend storage pool hostname', 'action': 'store', 'type': str, 'default': None}),
            (['-backendname'], {'help': 'volume backend name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def volume_backend_get(self):
        ...
        fields = ['name', 'capabilities.volume_backend_name', 'capabilities.utilization',
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-limit'], {'help': 'requests a page size of items. Use -1 to list all the volume', 'action': 'store',
                          'type': int, 'default': 20}),
    ...
    def volume_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.app.render(attachments, headers=['attachment_id', 'server_id', 'attached_at', 'host_name',
                                                      'device'])
                self.c('\nmetadata', 'underline')
            ...
            headers = ['id', 'name', 'os-vol-tenant-attr:tenant_id', 'size', 'status', 'type', 'bootable',
                       'created_at', 'attachments.0.server_id']
            fields = ['id', 'name', 'os-vol-tenant-attr:tenant_id', 'size', 'status', 'volume_type', 'bootable',
                      'created_at', 'attachments.0.server_id']
            print('Page: %s' % offset)
            print('Count: %s' % limit)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'volume name', 'action': 'store', 'type': str, 'default': None}),
            (['size'], {'help': 'volume size', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def volume_add(self):
        name = self.app.pargs.name
        size = self.app.pargs.size
        res = self.client.volume_v3.create(name=name, size=size)
        self.app.render({'msg': 'Create volume %s, id %s, size %sG' % (name, res['id'], res['size'])})

    @ex(
        help='migrate volume',
        description='migrate volume',
        arguments=OPENSTACK_ARGS([
            (['volume_id'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['host'], {'help': 'host', 'action': 'store', 'type': str, 'default': None}),
            (['-force_host_copy'], {'help': 'force_host_copy', 'action': 'store', 'type': str, 'default': "False"}),
            (['-lock_volume'], {'help': 'lock_volume', 'action': 'store', 'type': bool, 'default': "False"}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['-force'], {'help': 'force delete', 'action': 'store', 'type': str, 'default': 'false'})
        ])
    )
    def volume_del(self):
        oid = self.app.pargs.id
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['source_volume_id'], {'help': 'source volume id', 'action': 'store', 'type': str, 'default': None}),
            # (['source_volume_name'], {'help': 'source volume name', 'action': 'store', 'type': str, 'default': None}),
            (['name'], {'help': 'new volume name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'new volume description', 'action': 'store', 'type': str, 'default': None}),
            (['volume_type'], {'help': 'volume type id', 'action': 'store', 'type': str, 'default': None}),
            (['-bootable'], {'help': 'bootable value', 'action': 'store', 'type': str, 'default': 'true'}),
            (['-host'], {'help': 'source volume host', 'action': 'store', 'type': str, 'default': None}),
            (['-cluster'], {'help': 'source volume cluster', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def volume_manage(self):
        ...
        res = self.client.volume_v3.manage(source_volume_id, name, volume_type, bootable=bootable,
                                           desc=desc, availability_zone='nova', host=host, cluster=cluster)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['-status'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': 'available'}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['volume'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['project'], {'help': 'project id', 'action': 'store', 'type': str, 'default': None}),
            (['-volume_type'], {'help': 'the volume type id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'volume type id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    def volume_attachment_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'volume_id', 'instance', 'attach_mode', 'status', 'attached_at', 'detached_at']
            fields = ['id', 'volume_id', 'instance', 'attach_mode', 'status', 'attached_at', 'detached_at']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['instance'], {'help': 'instance id', 'action': 'store', 'type': str, 'default': None}),
            (['mountpoint'], {'help': 'instance mountpoint. ex. /dev/vda', 'action': 'store', 'type': str,
                              'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['-bootable'], {'help': 'bootable status', 'action': 'store', 'type': str, 'default': 'false'}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['metadata'], {'help': 'key:value string metadata', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'snapshot id', 'action': 'store', 'type': str, 'default': None}),
            (['metadata'], {'help': 'metadata key', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'snapshot id', 'action': 'store', 'type': str, 'default': None}),
            (['-volume'], {'help': 'volume id', 'action': 'store', 'type': str, 'default': None}),
            (['-group_snapshot'], {'help': 'group snapshot id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def volume_snapshot_get(self):
        ...
        if oid is not None:
            ...
            fields = ['id', 'name', 'descriptions', 'volume_id', 'os-extended-snapshot-attributes:project_id', 'status',
                      'created_at', 'os-extended-snapshot-attributes:progress', 'size']
            headers = ['id', 'name', 'desc', 'volume-id', 'project-id', 'status', 'created', 'progress', 'size']
            self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            ...
            (['-force'], {'help': 'indicates whether to snapshot, even if the volume is attached', 'action': 'store',
    ...
    def volume_snapshot_add(self):
        ...
        self.client.volume_v3.snapshot.create(**data)
        self.app.render({'msg': 'add volume %s snapshot %s' % (volume, name)}, maxsize=200)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'snapshot id', 'action': 'store', 'type': str, 'default': None}),
            (['-status'], {'help': 'snapshot status', 'action': 'store', 'type': str, 'default': 'available'})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'volume group id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'volume group type name', 'action': 'store', 'type': str, 'default': None}),
            (['-group_specs'], {'help': 'volume group type specs', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'volume group type id', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    def volume_group_get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'name', 'availability_zone', 'status', 'group_type', 'volume_types', 'volumes',
                       'created_at']
            fields = ['id', 'name', 'availability_zone', 'status', 'group_type', 'volume_types', 'volumes',
                      'created_at']
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['name'], {'help': 'volume group name', 'action': 'store', 'type': str, 'default': None}),
            (['-availability_zone'], {'help': 'volume group availability zone', 'action': 'store', 'type': str,
                                      'default': None}),
            (['volume_types'], {'help': 'the list of volume types. In an environment with multiple-storage back ends, '
                                        'the scheduler determines where to send the volume based on the volume type.',
                                'action': 'store', 'type': str}),
            (['group_type'], {'help': 'group type id', 'action': 'store', 'type': str})
        ])
    )
    def volume_group_add(self):
        ...
        self.client.volume_v3.group.create(**data)
        self.app.render({'msg': 'add volume group %s' % name}, maxsize=200)

    @ex(
        help='update volume group',
        description='update volume group',
        arguments=OPENSTACK_ARGS([
            (['id'], {'help': 'volume group type id', 'action': 'store', 'type': str}),
            (['-name'], {'help': 'volume group name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'volume group availability zone', 'action': 'store', 'type': str, 'default': None}),
            (['-del'], {'help': 'One or more volume UUIDs, separated by commas, that you want to remove from group',
                        'action': 'store', 'type': str, 'default': None}),
            (['-add'], {'help': 'One or more volume UUIDs, separated by commas, that you want to remove from group',
                        'action': 'store', 'type': str, 'default': None})
        ])
    )
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['-id'], {'help': 'volume group snapshot id', 'action': 'store', 'type': str, 'default': None}),
            (['-group'], {'help': 'volume group id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def volume_group_snapshot_get(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                res = self.client.volume_v3.snapshot.list(group_snapshot_id=oid)
                fields = ['id', 'name', 'descriptions', 'volume_id', 'os-extended-snapshot-attributes:project_id', 'status',
                          'created_at', 'os-extended-snapshot-attributes:progress', 'size']
                headers = ['id', 'name', 'desc', 'volume-id', 'project-id', 'status', 'created', 'progress', 'size']
                self.app.render(res, headers=headers, fields=fields)
    ...
    @ex(
        ...
        arguments=OPENSTACK_ARGS([
            (['group'], {'help': 'volume group id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'volume group snapshot name', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'volume group snapshot description', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
