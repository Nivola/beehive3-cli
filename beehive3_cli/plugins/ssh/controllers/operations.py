...
class SshOperationController(SshControllerChild):
    class Meta:
        ...
        stacked_type = 'nested'
        label = 'ops'
        description = "custom operations [DEPRECATED]"
        help = "custom operations [DEPRECATED]"
    ...
    def __node_run_cmd(self, scm, node, cmd, noresult=False):
        try:
            res = scm.sshcmd2node(node=node, user='root', cmd=cmd, timeout=10.0)
    ...
    @ex(
        help='check virtual machine boot disk is writable [DEPRECATED]',
        description='check virtual machine boot disk is writable [DEPRECATED]',
        arguments=PARGS([
            (['-node'], {'help': 'node id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'node name pattern', 'action': 'store', 'type': str, 'default': None}),
            (['-file'], {'help': 'node list in a file', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        help='check data domain mounted in dbaas [DEPRECATED]',
        description='check data domain mounted in dbaas [DEPRECATED]',
        arguments=PARGS([
            (['-node'], {'help': 'node id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'node name pattern', 'action': 'store', 'type': str, 'default': None}),
            (['-file'], {'help': 'node list in a file', 'action': 'store', 'type': str, 'default': None}),
    ...
    def dbaas_check_dd(self):
        ...
        cmds = [
            ...
            ('touch /bck_logici/xxxx && ls /bck_logici/xxxx && rm /bck_logici/xxxx', False)
        ]
        self.__node_run_cmds(scm, nodes, cmds, 'node-check.json')

    @ex(
        help='umount data domain mounted in dbaas [DEPRECATED]',
        description='umount data domain mounted in dbaas [DEPRECATED]',
        arguments=PARGS([
            (['-node'], {'help': 'node id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'node name pattern', 'action': 'store', 'type': str, 'default': None}),
            (['-file'], {'help': 'node list in a file', 'action': 'store', 'type': str, 'default': None}),
    ...
    @ex(
        help='mount data domain mounted in dbaas [DEPRECATED]',
        description='mount data domain mounted in dbaas [DEPRECATED]',
        arguments=PARGS([
            (['-node'], {'help': 'node id', 'action': 'store', 'type': str, 'default': None}),
            (['-name'], {'help': 'node name pattern', 'action': 'store', 'type': str, 'default': None}),
            (['-file'], {'help': 'node list in a file', 'action': 'store', 'type': str, 'default': None}),
    ...
    @ex(
        help='show action on dbaas [DEPRECATED]',
        description='show action on dbaas [DEPRECATED]',
        arguments=PARGS([
            (['file'], {'help': 'response file name', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    ...
    @ex(
        ...
        arguments=PARGS([
            (['ext_id'], {'help': 'openstack volume ext_id', 'action': 'store', 'type': str}),
    ...
    @ex(
        ...
        arguments=PARGS([
            (['opsvm'], {'help': 'openstack vm name', 'action': 'store', 'type': str}),
            (['-server'], {'help': 'if true update ext_id from openstack server to resource', 'action': 'store_true'}),
            (['-volume'], {'help': 'if true update ext_id in volume. Syntax volume_name:ext_id',
    ...
    def restore_vm(self):
        ...
        if len(ope_server) == 1:
            ...
            for v in volumes:
                ...
                print('   - ext_id: %s, id: %s, name: %s, type: %s, size: %s' % (rv['ext_id'], v['id'], v['name'],
                                                                                 v['type'], v['size']))
            print('------------------------------------------------')
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-type'], {'help': 'vm type. Can be mysql or oracle [default=mysql]', 'action': 'store', 'type': str,
                         'default': 'mysql'}),
            (['-ids'], {'help': 'dbaas uuids', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
