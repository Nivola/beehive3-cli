...
class SshUserController(SshControllerChild):
    class Meta:
        ...
        stacked_type = 'nested'
        label = 'node_users'
        description = "node user management [DEPRECATED]"
        help = "node user management [DEPRECATED]"
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'node user uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-username'], {'help': 'node user name', 'action': 'store', 'type': str, 'default': None}),
    ...
    def get(self):
        ...
        if oid is not None:
            ...
            headers = ['id', 'name', 'date', 'key', 'uid', 'gid', 'desc', 'home', 'shell']
            fields = ['id', 'name', 'date.creation', 'key', 'uid', 'gid', 'desc', 'home', 'shell']
    ...
    @ex(
        help='get node user password [DEPRECATED]',
        description='get node user password [DEPRECATED]',
        arguments=PARGS([
    ...
    def set_password(self):
        ...
        if propagate is True:
            res = self.run_cmd('echo -e "%s\n%s" | passwd %s' % (pwd, pwd, user['username']), node=node['id']).values()
    ...
    def __set_public_key(self, user, node, key_string):
        res = self.run_cmd('echo {key} >> /home/{name}/.ssh/authorized_keys'.format(name=user, key=key_string),
                           node=node).values()
        self.app.log.debug(res)

    @ex(
        help='add new ssh user [DEPRECATED]',
        description='add new ssh user [DEPRECATED]',
        arguments=PARGS([
            (['name'], {'help': 'user name', 'action': 'store', 'type': str}),
            (['-pwd'], {'help': 'user password', 'action': 'store', 'type': str, 'default': ''}),
            (['node'], {'help': 'node uuid', 'action': 'store', 'type': str}),
            (['-sshkey'], {'help': 'ssh key uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-desc'], {'help': 'user description', 'action': 'store', 'type': str, 'default': ''}),
            (['-attrib'], {'help': 'user attributes', 'action': 'store', 'type': str, 'default': ''}),
            (['-propagate'], {'help': 'if True create user also in node', 'action': 'store', 'type': str,
                              'default': 'false'}),
        ])
    )
    def add(self):
        ...
        if propagate:
            ...
            self.app.log.debug(res)
            res = self.run_cmd('mkdir -p /home/{name}/.ssh && chmod -R 700 /home/{name}/.ssh'.format(name=name),
                               node=node['id']).values()
            self.app.log.debug(res)
            res = self.run_cmd('echo '' > /home/{name}/.ssh/authorized_keys && chown -R {name}:{name} /home/{name}/.ssh'
    ...
    @ex(
        help='delete node user [DEPRECATED]',
        description='delete node user [DEPRECATED]',
        arguments=PARGS([
            (['id'], {'help': 'node user uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-propagate'], {'help': 'if True create user also in node', 'action': 'store', 'type': bool,
                              'default': False}),
        ])
    )
    def delete(self):
