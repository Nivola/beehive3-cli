...
class SshDbmsController(SshControllerChild):
    ...
    @ex(
        ...
        arguments=PARGS([
            (['host'], {'help': 'node name or uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-port'], {'help': 'node group name or uuid', 'action': 'store', 'type': int, 'default': 3306}),
            (['-type'], {'help': 'node group name or uuid', 'action': 'store', 'type': str, 'default': 'mysql'}),
            (['-user'], {'help': 'node group name or uuid', 'action': 'store', 'type': str, 'default': 'root'}),
            (['pwd'], {'help': 'node group name or uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-db'], {'help': 'node group name or uuid', 'action': 'store', 'type': str, 'default': 'mysql'}),
        ])
    )
    def ping(self):
