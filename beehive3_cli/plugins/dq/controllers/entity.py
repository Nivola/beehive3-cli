...
class DqResourceEntityController(BaseController):
    ...
    def check(self):
        ...
        if oid is not None:
            ...
            headers = {'idx': 'idx', 'id': 'id', 'name': 'name', 'container': 'container', 'parent': 'parent',
    ...
    def bad_get(self):
        ...
        headers = ['definition', 'id', 'name', 'container', 'parent', 'state', 'check', 'msg']
        fields = ['__meta__.definition', 'id', 'name', 'container', 'parent', 'state', 'check.check', 'check.msg']
    ...
    def compute_volume_repair(self):
        ...
        for item in items:
            ...
            if main is not None:
                ...
                # first search
                data = {'container': container, 'name': '%s-%s-volume' % (main['name'], container)}
                phyres = self.cmp_get(uri, data=data).get('resources', [])

                # second search
                if len(phyres) == 0:
                    pos = main['name'].find('-volume')
                    data = {'container': container, 'name': '%' + main['name'][:pos] + '%'}
                    phyres = self.cmp_get(uri, data=data).get('resources', [])
                ...
                for p in phyres:
                    ...
                    if p['name'].find('volume') > 0 and vol_size == size:
                        ...
                        find_num += 1
                if find_num == 0:
                    self.app.error('physical volume for %s in container %s does not exist' %
                                   (main['name'], container))
                    continue
                elif find_num > 1:
                    indata = input('\nrepair entity %s? ' % oid)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-hypervisor'], {'help': 'hypervisor', 'action': 'store', 'type': str, 'default': 'vsphere'}),
            (['-name'], {'help': 'name filter', 'action': 'store', 'type': str, 'default': 'vsphere'}),
        ])
    )
    def check_compute_instance(self):
        ...
        for instance in instances:
            ...
            volumes = details.pop('volumes')
            self.c('\nserver volumes', 'underline')
            self.app.render(volumes, headers=['uuid', 'name', 'storage', 'size', 'unit_number', 'bootable',
                                              'thin', 'mode'], maxsize=200)
