...
class AuthLdapController(BaseController):
    ...
    def pre_command_run(self):
        ...
        self.client = LdapAuth(conf.get('host'), SystemUser,
                               port=conf.get('port'),
    ...
    @ex(
        ...
        arguments=ARGS([
            ...
            (['-ldap'], {'help': 'ldap reference label', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def login(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['value'], {'help': 'value to search', 'action': 'store', 'type': str}),
            (['-filter'], {'help': 'search filter', 'action': 'store', 'type': str, 'default': 'mail'}),
            (['-fields'], {'help': 'query fields', 'action': 'store', 'type': str, 'default': 'cn,mail'}),
            (['-ldap'], {'help': 'ldap reference label', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def search(self):
    ...
    def infos(self):
        ...
        for item in values.split('\n'):
            ...
            if len(item) > 1:
                ...
                print('UPDATE `auth`.`user` SET `desc`=\'%s\', `email`=\'%s\' WHERE id=%s;' % (
                name, user['mail'], item[0]))
    ...
    @ex(
        ...
        arguments=ARGS([
            (['key'], {'help': 'user key like user email', 'action': 'store', 'type': str}),
            (['-filter'], {'help': 'search filter', 'action': 'store', 'type': str, 'default': '(mail={username})'}),
            (['-ldap'], {'help': 'ldap reference label', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def get(self):
