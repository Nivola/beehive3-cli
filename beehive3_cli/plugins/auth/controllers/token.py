...
class AuthTokenController(AuthChildController):
    class Meta:
        ...
        headers = ['token', 'type', 'provider', 'user', 'email', 'ip', 'ttl', 'timestamp']
        fields = ['token', 'type', 'provider', 'user', 'email', 'ip', 'ttl', 'timestamp']

    @ex(
        help='get tokens',
        description='get tokens',
        arguments=PARGS([
            (['-id'], {'help': 'token uuid', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['-type'], {'help': 'can be keyauth, oauth2. oauth2 create a token using a jwt client', 'action': 'store', 
                         'type': str, 'default': 'keyauth'}),
            (['user'], {'help': 'login user', 'action': 'store', 'type': str}),
            (['pwd'], {'help': 'login password', 'action': 'store', 'type': str}),
            (['-client'], {'help': 'ouath2 client uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-sub'], {'help': 'sub field for oauth2 jwt login', 'action': 'store', 'type': str, 'default': None})
        ])
    )
    def add(self):
        ...
        if auth_type == 'keyauth':
            ...
            private_key = binascii.a2b_base64(client['private_key'])
    ...
    @ex(
        ...
        arguments=ARGS([
            (['id'], {'help': 'token uuid or all for all', 'action': 'store', 'type': str}),
        ])
    )
    def delete(self):
