...
class AuthChildController(BaseController):
    class Meta:
        ...
        user_headers = ['uuid', 'name', 'email', 'desc', 'active', 'creation', 'modified', 'last_login']
        user_fields = ['uuid', 'name', 'email', 'desc', 'active', 'date.creation', 'date.modified', 'date.last_login']

        role_headers = ['uuid', 'name', 'alias', 'active', 'creation', 'modified', 'expiry']
        role_fields = ['uuid', 'name', 'alias', 'active', 'date.creation', 'date.modified', 'date.expiry']

        group_headers = ['uuid', 'name', 'desc', 'active', 'creation', 'modified', 'expiry']
        group_fields = ['uuid', 'name', 'desc', 'active', 'date.creation', 'date.modified', 'date.expiry']

        perm_headers = ['permission-id', 'object-id', 'authorization-id', 'subsystem', 'type', 'action-id', 'action']
...
class ProviderController(AuthChildController):
    ...
    def get(self):
        uri = u'%s/providers' % self.baseuri
        res = self.cmp_get(uri)
        self.app.render(res, key=u'providers', headers=[u'type', u'name'])
