...
class AuthObjectController(AuthChildController):
    ...
    @ex(
        ...
        arguments=PARGS([
            (['id'], {'help': 'permission id', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get(self):
    ...
    def get_actions(self):
        ...
        res = self.cmp_get(uri)
        self.app.render(res, key='object_actions', headers=self._meta.act_headers, fields=self._meta.act_fields)
    ...
    @ex(
        ...
        arguments=PARGS([
            ...
            (['-subsystem'], {'help': 'subsystem', 'action': 'store', 'type': str, 'default': None}),
            (['-type'], {'help': 'type', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get_types(self):
        ...
        self.app.render(res, key='object_types', headers=self._meta.type_headers, fields=self._meta.type_fields)
    ...
    @ex(
        ...
        arguments=PARGS([
            (['-id'], {'help': 'account uuid', 'action': 'store', 'type': str, 'default': None}),
            (['-objid'], {'help': 'autorization id', 'action': 'store', 'type': str, 'default': None}),
            (['-subsystem'], {'help': 'subsystem', 'action': 'store', 'type': str, 'default': None}),
            (['-type'], {'help': 'type', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get_objects(self):
        ...
        if oid is not None:
            ...
            if self.is_output_text():
                ...
                self.app.render(perms, headers=['permission-id', 'action-id', 'action'], fields=['id', 'aid', 'action'])
            else:
                self.app.render(res, key='object', details=True)
            ...
            res = self.cmp_get(uri, data=data)
            self.app.render(res, key='objects', headers=self._meta.obj_headers, fields=self._meta.obj_fields)

    @ex(
        help='get method',
        description='get method',
        arguments=PARGS([
            (['rule'], {'help': 'method url/rule ', 'action': 'store', 'type': str, 'default': None}),
            (['-subsystem'], {'help': 'subsystem', 'action': 'store', 'type': str, 'default': None}),
        ])
    )
    def get_method(self):
    ...
    @ex(
        ...
        arguments=ARGS([
            (['desc'], {'help': 'user description', 'action': StringAction, 'type': str, 'nargs': '+', 'default': ''}),
            (['objid'], {'help': 'autorization id', 'action': 'store', 'type': str, 'default': None}),
            (['subsystem'], {'help': 'subsystem', 'action': 'store', 'type': str, 'default': None}),
            (['type'], {'help': 'type', 'action': 'store', 'type': str, 'default': None}),
        ])
    ...
    @ex(
        ...
        arguments=ARGS([
            (['ids'], {'help': 'comma separated list of object id', 'action': 'store', 'type': str}),
        ])
    )
    def del_object(self):
