...
BASE_ARGS = [
    ...
    (['--curl'], {'action': 'store_true', 'dest': 'curl', 'help': 'Print api request as curl command in console log'}),
]
...
class BaseController(CliController):
    ...
    def cmp_post(self, uri, data='', headers=None, timeout=120, task_timeout=600, delta=2, task_key=None):
        res = self.api.call(uri, 'POST', data=data, headers=headers, timeout=timeout)
        self.__wait_task(res, task_timeout, delta, task_key)
        return res

    def cmp_put(self, uri, data='', headers=None, timeout=120, task_timeout=600, delta=2, task_key=None):
        res = self.api.call(uri, 'PUT', data=data, headers=headers, timeout=timeout)
        self.__wait_task(res, task_timeout, delta, task_key)
        return res

    def cmp_patch(self, uri, data='', headers=None, timeout=120, task_timeout=600, delta=2, task_key=None):
        res = self.api.call(uri, 'PATCH', data=data, headers=headers, timeout=timeout)
        self.__wait_task(res, task_timeout, delta, task_key)
        return res

    def cmp_delete(self, uri, data='', headers=None, timeout=120, confirm=True, entity='', task_timeout=600, delta=2,
