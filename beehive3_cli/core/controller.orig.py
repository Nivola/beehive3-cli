...
from cement import Controller, minimal_logger, FrameworkError
from cement.ext.ext_argparse import _clean_func
from six import ensure_binary
from beecell.simple import merge_list, dict_get
...
BASE_ARGS = [
    ...
    (['-e', '--env'], {'action': 'store', 'dest': 'env', 'help': 'Execution environment', 'default': None}),
    (['-f'], {'action': 'store', 'dest': 'format', 'help': 'Output format', 'default': 'text'}),
    (['-k', '--key'], {'action': 'store', 'dest': 'key',
                       'help': 'Secret key file to use for encryption/decryption', 'default': None}),
    (['--vault'], {'action': 'store', 'dest': 'vault',
                   'help': 'Ansible vault password to use for inventory decryption', 'default': None}),
    (['--time'], {'action': 'store_true', 'dest': 'time', 'help': 'Print command execution time'}),
    (['--notruncate'], {'action': 'store_true', 'dest': 'notruncate', 'help': 'Disable truncate of output'}),
    (['--curl'], {'action': 'store_true', 'dest': 'curl', 'help': 'Print api requst as curl command in console log'}),
]
...
PAGINATION_ARGS = [
    ...
    (['-field'], {'help': 'list sort field [default=id]', 'action': 'store', 'type': str, 'default': 'id'}),
...
class CliController(Controller):
    ...
    def _process_commands(self, controller):
        ...
        for command in commands:
            ...
            default_contr_func = "%s.%s" % (command['controller']._meta.label, command['func_name'])
    ...
    def c(self, data, style):
        fn = getattr(ColoredText(), style)
        print(fn(data))
...
class BaseController(CliController):
    ...
    def configure_cmp_api_client(self):
        ...
        self.api = CmpApiClient(self.app, self._meta.cmp.get('subsystem'), self._meta.cmp.get('baseuri'),
                                self.key)
    ...
    def cmp_get(self, uri, data='', headers=None, timeout=60):
        res = self.api.call(uri, 'GET', data=data, headers=headers, timeout=timeout)
        return res

    def cmp_post(self, uri, data='', headers=None, timeout=60, task_timeout=600, delta=2, task_key=None):
        res = self.api.call(uri, 'POST', data=data, headers=headers, timeout=timeout)
        self.__wait_task(res, task_timeout, delta, task_key)
        return res

    def cmp_put(self, uri, data='', headers=None, timeout=60, task_timeout=600, delta=2, task_key=None):
        res = self.api.call(uri, 'PUT', data=data, headers=headers, timeout=timeout)
        self.__wait_task(res, task_timeout, delta, task_key)
        return res

    def cmp_patch(self, uri, data='', headers=None, timeout=60, task_timeout=600, delta=2, task_key=None):
        res = self.api.call(uri, 'PATCH', data=data, headers=headers, timeout=timeout)
        self.__wait_task(res, task_timeout, delta, task_key)
        return res

    def cmp_delete(self, uri, data='', headers=None, timeout=60, confirm=True, entity='', task_timeout=600, delta=2,
                   output=True, task_key=None):
